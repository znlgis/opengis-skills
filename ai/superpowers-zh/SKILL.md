...
包内含 `skills/`、`agents/`、`commands/`、`hooks/` 与一键安装脚本 `bin/superpowers-zh.js`。它在 obra/superpowers（英文上游，250k+ ⭐）基础上完整汉化（14 个翻译 skills）并新增 4 个中国原创 skills（另有 2 个上游历史保留 skills，全仓共 20 个），支持 Claude Code / GitHub Copilot CLI / Hermes Agent / Cursor / Claw Code / Windsurf / Kiro / Gemini CLI / OpenCode / Qoder 等 26 款工具。
...
| 中国特色 Skills | 4 个中国原创（`chinese-code-review` / `chinese-git-workflow` / `chinese-documentation` / `chinese-commit-conventions`，需 `/chinese-*` 手动调用）+ 2 个上游历史保留（MCP 构建器 / 工作流执行器） |
...
| 支持哪些工具 | Claude Code、Copilot CLI、Cursor、OpenCode、Hermes Agent、Windsurf、Gemini CLI、Kiro、Qoder 等 26 款 |
| 和英文 superpowers 区别 | 完整汉化（14 个翻译 skills）+ 4 个中国原创 skills（中文代码审查 / Git 工作流 / 技术文档 / 提交规范）+ 2 个上游历史保留（MCP 构建器 / 工作流执行器），共 20 个 skills |
