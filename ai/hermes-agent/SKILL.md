...
- **自学习闭环**：使用中自主**创建技能（skills）**、在使用时**改进技能**、把关键事实写入**持久化记忆**、检索自己的**历史会话**，并对用户建立越来越精确的画像。
- **终端优先**：CLI/TUI（`hermes`）交互，命令如 `hermes setup`、`hermes model`、`hermes tools`、`hermes config`、`hermes doctor`。
