...
Admin.NET 前端基于 **vue-next-admin** 模板二次开发，采用 **Vue 3 Composition API** + **TypeScript** + **Element Plus** + **Vite 5** 技术栈。项目提供完整的企业级中后台管理界面，包括登录认证、动态菜�?路由、RBAC 权限控制、多语言、主题切换、SignalR 实时通讯等能力�?
...
�?  ├── api-services/              # 自动生成�?API 服务（对应后�?Swagger�?
�?  ├── api-plugins/               # 插件相关�?API 服务
�?  ├── assets/                    # 静态资源（图片、样式）
�?  ├── components/                # 全局公共组件
...
#### 1. 自动生成�?API 服务（`src/api-services/`�?

通过 `api_build/` 脚本从后�?Swagger JSON 自动生成 TypeScript API 客户端：
...
Admin.NET 前端采用 **动态路�?* 方案，菜单和路由信息从后端接口获取：
...
前端通过 `@microsoft/signalr` 与后�?Hub 建立长连接：
    ...
    <el-card shadow="hover">
      <el-form :model="queryParams" :inline="true">
        <el-form-item label="名称">
          <el-input v-model="queryParams.name" placeholder="请输入名�? clearable />
        </el-form-item>
        <el-form-item>
...
| `md-editor-v3` | md-editor-v3 | Markdown 编辑�?|
| `monaco-editor` | Monaco Editor | 代码编辑�?|
| `echarts` / `echarts-gl` | ECharts 6 | 图表�?3D 可视�?|
| `vform3-builds` | VForm3 | 在线表单构建�?|
| `vue-plugin-hiprint` / `print-js` | hiprint | 打印与报�?|
| `@vue-office/pdf` / `excel` / `docx` | vue-office | Office 文件预览 |
| `logicflow` | LogicFlow | 流程图编�?|
| `relation-graph` | relation-graph | 关系图可视化 |
| `json-editor-vue` | json-editor-vue | JSON 编辑�?|
...
- **CSS 预处理器�?* SASS
- **主题方案�?* Element Plus 主题变量 + 自定�?CSS 变量
- **暗黑模式�?* 支持亮色/暗色主题切换
...
3. **API 对接**：后�?Swagger 更新后运�?`api_build` 脚本自动生成 TypeScript API 客户�?�?�?`getAPI(XxxApi)` 调用
4. **权限控制**：后端菜单配置按钮权限标�?�?前端�?`v-auth="'xxx:action'"` 指令控制按钮显示
5. **构建部署**：`pnpm run build` �?`dist/` 产物 �?Nginx 部署（SPA try_files + API 代理 + WebSocket /hubs�?

### 关键模式与常见陷�?

- **包管理器**：必须用 `pnpm`，项目依�?`pnpm-workspace.yaml`，npm/yarn 可能失败
- **环境变量前缀**：Vite 项目中自定义环境变量必须�?`VITE_` 开头，否则 `import.meta.env` 读不�?
- **路由组件路径**：后端菜单配置的组件路径对应 `src/views/` 下的文件路径（不�?`.vue` 后缀），大小写敏�?
- **权限标识一致�?*：前�?`v-auth` 指令中的标识必须与后端菜单配置中的按钮权限标识完全一�?
- **API 自动生成**：修改后端接口后务必重新运行 `api_build` 脚本，否则类型定义过�?
- **SignalR 代理**：Nginx 需配置 WebSocket 代理（`/hubs` 路径），否则实时通知不工�?
...
| 独立 Vue3 后台项目 | 直接�?vue-next-admin 模板 + Element Plus |
| 仅需简单管理界�?| vue-pure-admin / naive-ui-admin |
| 移动�?小程�?| uni-app / Taro |
...
1. **包管理器**：务必使�?`pnpm`，项目使�?`pnpm-workspace.yaml` 管理
2. **API 自动生成**：修改后端接口后需重新运行 `api_build` 脚本更新前端 API 类型定义
3. **权限标识**：前�?`v-auth` 指令中使用的权限标识需与后端菜单配置中的按钮权限一�?
4. **路由组件路径**：后端菜单配置的组件路径对应 `src/views/` 下的文件路径（不含后缀�?
5. **环境变量前缀**：Vite 项目中自定义环境变量必须�?`VITE_` 开�?
6. **国密加密**：登录密码等敏感数据前端使用 `sm-crypto-v2` �?SM2 加密后传�?
7. **SignalR 连接**：需正确配置 Nginx 代理 WebSocket（`/hubs` 路径�?
8. **Node 版本**：要�?Node.js >= 18.0.0
...
- **furion** �?后端使用�?.NET Web 框架，前端通过其动�?API 自动生成接口：[../furion/SKILL.md](../furion/SKILL.md)
