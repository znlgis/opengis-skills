...
- **零配置启�?*：`Inject()` + `AddInject()` 自动激�?
- **动�?API**：实�?`IDynamicApiController` �?自动暴露 RESTful 接口
- **规范化结�?*：统一 `RESTfulResult<T>` 响应
- **依赖注入**：`ITransient/IScoped/ISingleton` 标记接口自动注册
- **数据�?*：内�?SqlSugar �?EF Core 双方�?
- **鉴权**：`[Authorize]` + JWT + 策略
- **远程请求**：`IHttpRemote` + `[HttpRequest]` 配置�?
- **定时任务**：`Furion.Schedule`（基�?Quartz 类似的语法）
- **事件总线**：本�?+ RabbitMQ
...
EF Core �?`Furion.DatabaseAccessor`�?
...
```csharp
builder.Services.AddJwt<JwtHandler>();   // 自定�?IAuthorizationHandler
```
...
```csharp
[JobDetail("job1", Description = "每分钟一�?)]
...
1. **快速起�?*：`dotnet new webapi` �?添加 Furion �?�?`builder.Services.AddControllers().AddInject()` �?`app.UseInject()`
2. **定义服务**：实�?`IDynamicApiController` + 标记接口（`ITransient`/`IScoped`/`ISingleton`）→ 接口自动暴露
3. **配置数据�?*：二选一 �?SqlSugar（`AddSqlSugar`）或 EF Core（`AddDatabaseAccessor`�?
4. **鉴权**：配�?`JWTSettings` + `AddJwt<JwtHandler>()` + `[Authorize]`
5. **扩展**：远程请求用 `IHttpDispatchProxy`，定时任务用 `IJob`，事件总线�?`IEventSubscriber`

### 关键模式与常见陷�?

- **`Inject()` 三步�?*：`builder.Inject()` �?`builder.Services.AddControllers().AddInject()` �?`app.UseInject()`，缺一不可
- **动�?API 命名**：方法名决定 HTTP 动词（`Get`/`Post`/`Add`/`Update`/`Delete`/`Page`），不匹配时�?`[HttpPost]` 覆盖
- **DI 标记接口**：`ITransient`/`IScoped`/`ISingleton` 三选一，Abstract 类不生效
- **规范化响�?*：所有接口默认包装为 `{statusCode, succeeded, data, errors}`，可通过配置关闭
- **Swagger 不显�?*：检�?`app.UseInject()` 是否正确调用，以�?`SpecificationDocumentSettings` 分组配置

### 如何选择正确方案

| 场景 | 推荐方案 |
|------|---------|
| 快速构建单�?API | Furion 动�?API |
| 已有大型项目渐进迁移 | 仅用 Furion �?DI/配置/事件总线 |
| 微服务架�?| Furion + 自建网关 |
| 前后端分离后�?| Furion（后端）+ Admin.NET 前端模板 |
...
| 问题 | 解决 |
|------|------|
| 接口未暴�?| 服务实现 `IDynamicApiController` �?`AddInject()` |
| Swagger 不显�?| 启用 `app.UseInject()`；检�?`SpecificationDocumentSettings.GroupOpenApiInfos` |
| DI 不生�?| 实现类需�?`ITransient/IScoped/ISingleton` 之一 |
| 单元测试 | 使用 `Furion.UnitTesting` 包，或调�?`App.Initialize()` |
| `App` 未初始化 | 异步入口需 `Inject()` 完成�?|

---

## 相关技�?

- **sqlsugar** �?Furion 默认推荐�?ORM，提供链式查询与多数据库支持：[../sqlsugar/SKILL.md](../sqlsugar/SKILL.md)
