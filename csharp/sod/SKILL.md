...
> **项目地址�?* <https://github.com/znlgis/SOD>（社区主仓库�?<https://gitee.com/znlgis/SOD>�?
>
...
SOD �?PDF.NET 演化而来，主要特点：

- **混合 ORM 模式**：原�?SQL + ORM + SQL-MAP（XML 配置�? OQL（对象查询语言�?
- **多数据库**：SqlServer / Oracle / MySQL / PostgreSQL / SQLite / Access / 达梦 DM / 人大金仓 / 神舟通用
- **AdoHelper**：屏蔽底�?Provider 差异
- **实体�?*：`EntityBase`，支持脏字段跟踪、字段映射、状态机
- **OQL**：链�?API 查询、连接、分组、聚�?
- **SQL-MAP**：XML �?SQL，Mybatis 风格
- **读写分离**：主从配�?
- **缓存与监�?*：内�?SQL 性能日志
...
可通过 SOD CodeMaker �?`EntityBuilder` 自动生成�?
...
通过 SOD 工具（SqlMapTool）从 XML 自动生成强类�?DAL 类�?
...
写走 master，读�?slave；或借助 `IReadDb` / `IWriteDb` 接口�?
...
- 启用 `MyDB.SQL_LOG_INFO` 输出�?SQL
- `db.CommandTimeOut`
- 连接池大小通过连接字符�?
- OQL 大数据量分页�?`Limit()`，不�?ToList 全量
...
1. **建实体类**：继�?`EntityBase`，设�?`TableName`、`IdentityName`、`PrimaryKeys`，属性用 `getProperty<T>()` / `setProperty()` 
2. **选查询方�?*：简�?CRUD �?`EntityQuery<T>`；复杂条�?�?OQL 链式 API；固�?SQL �?SQL-MAP XML
3. **多数据库切换**：仅改连接字符串�?`providerName`（SqlServer / MySql / PostgreSQL / DaMeng 等）
4. **读写分离**：配置主从连接字符串，写�?master，读�?slave
5. **事务**：同一 `AdoHelper` 实例�?`BeginTransaction()` �?操作 �?`Commit()` / `Rollback()`

### 关键模式与常见陷�?

- **实体属性声�?*：必须用 `getProperty<T>(name)` / `setProperty(name, value, size)`，不能用自动属�?
- **自增主键回填**：`IdentityName` 必须设置正确，否�?INSERT �?`Id` 不更�?
- **脏字段跟�?*：`Update()` 只提交变更字段（调用 `setProperty` 的字段），性能优于全量更新
- **OQL 分页**：用 `oql.Limit(pageSize, pageIndex)` 分页，不�?ToList 后手动分�?
- **SQL-MAP 工具�?*：用 SOD �?SqlMapTool �?XML 生成强类�?DAL，手�?XML 容易拼错

### 如何选择正确方案

| 场景 | 推荐方案 |
|------|---------|
| 政企项目（达�?金仓/神通） | SOD（多数据库支持最全） |
| 追求开发效�?| SqlSugar（链式查询更现代�?|
| MyBatis 用户迁移 | SOD SQL-MAP 模式 |
| 新项�?| 评估 SqlSugar vs SOD 的数据库支持列表 |
...
| 自增主键未填�?| `IdentityName` 设置正确 |
| 中文乱码 | 数据库用 utf8mb4 / NVARCHAR |
| 事务跨连接失�?| 使用同一 `AdoHelper` 实例 |
