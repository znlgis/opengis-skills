...
- 全新偏移引擎：更鲁棒 -- ?mitre/square/round 端帽与连 -- ?
- 简 -- ?API：`Union/Intersect/Difference/Xor` 顶级函数
...
# 或直 -- ?add subdirectory CPP/Clipper2Lib  -- ?CMake
```
...
```csharp
ClipperD clipper = new(2);   // 2 = 浮点保留 2 位小数精 -- ?
...
```csharp
double area = Clipper.Area(path);                       // 有符号面 -- ?
PointD c    = Clipper.Centroid(path);                   // 形心
RectD bbox  = Clipper.GetBounds(paths);                 // 包围 -- ?

PathsD simp = Clipper.SimplifyPaths(paths, 0.5);        // 简化（容差 -- ?
PathsD ramer = Clipper.RamerDouglasPeucker(paths, 0.5); // RDP 简 -- ?
...
1. 通过 `Clipper.MakePath` 或手工构 -- ?`PathD`/`Path64` 列表
2. 选择 FillRule（一般用 `NonZero` -- ?
3. 直接调用静态函 -- ?`Clipper.Union(a, b, FillRule.NonZero)`
4. 如需 PolyTree（孔洞层级），使 -- ?`ClipperD`  -- ?+ `PolyTreeD`
5. 对结果进 -- ?`SimplifyPaths`  -- ?`TrimCollinear` 清理
6. 导出结果，转换为下游格式（如转回 DXF polyline -- ?

### 工作流二：多边形膨胀与缩进（CAM 刀路补偿）

1.  -- ?CAD 文件读取零件轮廓多边 -- ?
2. 使用 `Clipper.InflatePaths(paths, offset, JoinType.Round, EndType.Polygon)` 生成刀路偏 -- ?
3. 正值外扩（粗加工），负值内缩（精加工）
4. 对偏移结果做 `SimplifyPaths`，去除多余顶 -- ?
5. 输出 -- ?G-code  -- ?CAM 系统格式

---

## 性能要点

1. **优先 Path64 + 适当 scale**：浮点版内部仍用整数实现，避免精度问 -- ?
2. **SimplifyPaths** 在大量数据布尔前预处理，减少顶点
3. **避免共线 -- ?*：`TrimCollinear`
4. **大对象用 PolyTree**：避免重新计算外/内环关系
5. **Inflate 注意 miter limit**：尖角处可能产生超大顶点
6. **多线 -- ?*：Clipper2 单实例非线程安全，多线程需独立实例
...
| 问题 | 解决 |
|------|------|
| 输出空集 | 检 -- ?FillRule、方向（逆时 -- ?vs 顺时针）、闭合 -- ?|
| 偏移变形 | 调整 `JoinType`  -- ?`MiterLimit` |
| 浮点精度异常 | 改用 `Path64`  -- ?`ClipperD(precision)` |
|  -- ?Clipper1 不兼 -- ?| API 已变化，参考迁移指 -- ?|
...
- **推荐工作流模 -- ?*：新项目优先使用 Clipper2 的静态顶层函数（`Union`/`Intersect`/`Difference`/`Xor`/`InflatePaths`）， -- ?Clipper1 的类实例方式更简洁。浮点模式直接用 `PathsD`，整数模式用 `Paths64`（指 -- ?scale） -- ?
- **关键注意事项**：① `FillRule` 默认 -- ?`NonZero`；② `InflatePaths`  -- ?`JoinType`  -- ?`MiterLimit` 影响偏移质量——圆形连接（Round）最稳健；③ 浮点模式 -- ?`ClipperD(precision)`  -- ?precision 参数控制内部整数转换精度；④ `PolyTree` 保留孔洞层级 -- ?
...
- **clipper1**  -- ?一 -- ?Clipper 库（遗留代码维护参考）：[../clipper1/SKILL.md](../clipper1/SKILL.md)
- **qcad**  -- ?2D CAD 软件，DXF 多边形编辑：[../qcad/SKILL.md](../qcad/SKILL.md)
- **librecad**  -- ?开 -- ?2D CAD，配 -- ?Clipper 做几何处理：[../librecad/SKILL.md](../librecad/SKILL.md)
