...
> **许可证：** GPL-3.0+
>
> **最新稳定版：** 3.2（2026-03 发布，见 [GitHub Releases](https://github.com/solvespace/solvespace/releases)）
...
新建文件 (.slvs)
  ...
  │       ├─ 直线 (L) / 圆 (C) / 弧 (A) / 矩形 (R) / 样条
  │       └─ 添加约束：距离 (D) / 角度 (N) / 平行 / 对称 / 等长 / 重合
  │
...
| 键 | 功能 |
|----|------|
| `L` `C` `R` `A` | 直线 / 圆 / 矩形 / 圆弧工具（单键直接启动） |
| `D` | Distance 约束 |
| `N` | Angle / Equal Angle 约束（3.2 起改为 `N`） |
| `L` | Parallel / Tangent 约束（选中实体时生效） |
| `Q` | Equal Length / Radius 约束 |
| `H` | Horizontal |
| `V` | Vertical |
| `Y` | Symmetric 约束 |
| `Ctrl+Tab` | 显示/隐藏工具栏（3.2 起） |
...
solvespace-cli thumbnail --output out.png --view top --size 1024 input.slvs
solvespace-cli export-mesh --output out.stl input.slvs                  # 输出 STL
solvespace-cli export-view --output out.svg --view top input.slvs
```

通用形式为 `solvespace-cli <command> <options> <filename> [filename...]`，输出文件一律用 `--output` 指定。
...
3. 求解失败：减少约束、检查冗余；修改几何或约束值后会自动重新求解
...
- **常用代码模式**：GUI：按 `L` 画线、`C` 画圆 → 选中实体后 `D` 距离约束、`N` 角度约束 → New Group → Extrude 拉伸。CLI：`solvespace-cli regenerate input.slvs` / `solvespace-cli export-mesh --output out.stl input.slvs`。
...
1. 新建文件（.slvs），在 XY Workplane 上按 `L` 画轮廓线
2. `D` 添加距离约束，`N` 添加角度约束，`Q` 添加等长约束，直到草图全黑（完全约束）
