...
- **基于 Three.js / Babylon.js** 的高层封 -- ?
- **场景编辑 -- ?*：可视化拖拽、属性面板、节点树
- **资产管理**：模型库、材质库、HDR 环境
- **动画与状态机**：物体动画、相机路径、时间轴
- **交互**：射线拾取、点击、悬浮、UI 弹窗
- **数据接入**：MQTT、WebSocket 实时推 -- ?
- **导出**：JSON 场景、glTF、嵌 -- ?SDK

> 不同发行版（开源版 / 商业 -- ?/ 企业版）功能差异较大，本 SKILL 聚焦开源版与通用 SDK 用法 -- ?
...
1. 新建场景  -- ?选模板（ -- ?/ 工业园区 / 数据中心 -- ?
2. 拖入模型库资 -- ?
3. 编辑材质、灯光、HDR
4. 设置交互（点击事件、动画轨道）
5. 导出 `.json` 场景文件  -- ? -- ?Viewer 加载
...
1. 使用 Astral3D 编辑器新建场景，拖入厂房模型（glTF），布置灯光 -- ?HDR 背景
2. 为设备节点命名（ -- ?`sensor_A1`），配置交互弹窗模板
3. 导出 `factory.json` 场景文件
4. 在前端页面实例化 `Viewer`，加载场景文 -- ?
5. 通过 `MqttBridge` 连接 MQTT Broker，订阅设备遥测主 -- ?
6. 在回调中根据设备 ID 更新对应节点的颜 -- ?透明 -- ?标签

### 工作流二：Web 端轻量化 CAD 预览

1. 从服务端获取 STEP/IFC 文件 URL
2. 使用 `ModelLoader.load()` 加载模型（底层调 -- ?OCCT.js 解析 -- ?
3. 将模型加入场景，调整位置、缩 -- ?
4. 添加 OrbitControls 供用户旋 -- ?缩放查看
5. 可选：添加剖面裁剪（clipping plane）查看内部结 -- ?
...
2. **静态合 -- ?*：场景导出时启用 `mergeStatic`
3. **LOD**：编辑器为模型生 -- ?3  -- ?LOD
4. **物理光照贴图**：烘焙到资产 -- ?
5. **Frustum culling + occlusion culling** 默认开 -- ?
6. **Worker 解码**：DRACO/KTX2/FBX 使用独立 Worker
7. **PBR 关闭高反射时 -- ?`unlit` 材质**
...
| 问题 | 解决 |
|------|------|
| 模型加载失败 | 检 -- ?CORS；统一 -- ?glTF 2.0 |
| 中文按钮乱码 | 字体子集中包 -- ?CJK |
| 实时推送延 -- ?| MQTT QoS 0；服务器  -- ?浏览 -- ?WebSocket 直连 |
| 移动端发 -- ?| 关闭实时阴影、降低分辨率 `viewer.setPixelRatio(1)` |
| 相机穿 -- ?| 启用碰撞或限 -- ?minDistance |

---

## AI 使用建议

- **推荐工作流模 -- ?*：AI 助手应优先使用编辑器导出 -- ?`.json` 场景文件作为基础，再通过 API 动态注入数据和交互——避免在代码中手动构造复杂场景树 -- ?
- **关键注意事项**：① MQTT/WebSocket 连接需处理断线重连；② 模型路径需确保 CORS 配置正确；③ glTF  Draco 压缩后的模型需 -- ?DRACOLoader 支持；④ 移动端需主动降低 `pixelRatio` 与关闭阴影 -- ?
- **常用代码模式**：场景加 -- ? -- ?数据源接 -- ? -- ?`viewer.scene.findById()` 定位节点  -- ?更新节点属性（颜色/位置/可见性）。对于新模型，优先使 -- ?`ModelLoader.load()` 而非底层 Three.js API -- ?
...
- **lightcad**  -- ?轻量 Web 2D CAD 框架 -- ?DXF 编辑：[../lightcad/SKILL.md](../lightcad/SKILL.md)
...
> 官方仓库（github.com/AstralEngine/astral3d）已404不可用，请以 znlgis.github.io 中的最新链接为准。
