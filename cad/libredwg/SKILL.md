...
- **写入版本**：稳定支�?R1.4 �?R2004（R2007+ 写入仍试验性）
- **读写**：DWG �?DXF（ASCII/Binary），并能�?JSON/XML/SVG
- **命令行工�?*：`dwgread`、`dwgwrite`、`dwg2dxf`、`dxf2dwg`、`dwg2SVG`、`dwgrewrite`
- **绑定**：C API、Python（`libredwg.python`）、Common Lisp、Perl

> 当前稳定�?**0.13.3**�?024-02）。与 ODA / Teigha 的差异：LibreDWG 完全开源（GPL），但版本兼容范围比商业库窄、高版本写入支持仍在完善�?
...
```bash
# Debian/Ubuntu
sudo apt install libredwg-dev libredwg0      # �?+ 头文�?
...
# 重写（修�?版本转换�?
dwgrewrite -v 2018 input.dwg out.dwg       # 转换�?R2018
```
...
> 写入 API 仍在演化，建议参�?`programs/` 下示例与最�?API�?
...
适合�?DWG 中的多段�?多边形转�?PostGIS / QGIS�?

---

## 性能与精�?

1. 大文件用 `--no-check`、`-O DXFB`（二进制 DXF）减小体�?
2. 读取后通过 `dwg_get_BLOCKS_HEADER` 等高�?API，避免遍历所�?objects
3. 写入时显式设�?`dwg.header.version`
4. 检�?`dwg_errstrings` �?`dwg_set_loglevel` 排查
...
| 问题 | 解决 |
|------|------|
| 高版�?DWG 无法�?| 等待 LibreDWG 支持，或先用 ODA File Converter 转为 R2010 |
| 写入失真 | 启用 `--enable-write` 编译；优先使�?DXF 出口 |
| 中文文字乱码 | DWG 编码�?ANSI/CP936；`dwgread -c utf8` 切换 |
| 引用块缺�?| 解析 `INSERT` 时跟�?`BLOCK_HEADER` 引用 |

---

## AI 使用建议

- **推荐工作流模�?*：AI 助手应根据场景选择 LibreDWG 的使用层级——简单转换用 CLI（`dwg2dxf`/`dwgread`），数据提取�?JSON/GeoJSON 输出，复杂处理用 C API �?Python 绑定。写入功能仍在完善，�?DWG 前考虑先写 DXF 再转换�?
- **关键注意事项**：① 高版�?DWG（R2021+）不被支持，先用 ODA File Converter 转为 R2010/R2018；② 中文文字编码默认�?ANSI/CP936，读取时可用 `dwgread -c utf8`；③ 写入需编译时启�?`--enable-write`；④ 大文件用 `--no-check` 加速读取�?
...
- **librecad** �?开�?2D CAD，DXF 编辑�?LibreDWG 协同使用：[../librecad/SKILL.md](../librecad/SKILL.md)
...
1. `dwg2dxf -y input.dwg` �?DWG 转为 DXF ASCII 格式
2. `dwgread -O JSON input.dwg -o out.json` 提取结构化数�?
3. 解析 JSON，遍历模型空间实体（LINE/CIRCLE/INSERT/TEXT 等）
4. 将提取的几何数据转入下游系统（GIS/数据�?可视化）
5. 对于 GIS 用途：`dwgread -O GeoJSON input.dwg -o out.geojson` 直接生成 GeoJSON

### 工作流二：以 C 程序读写 DWG 文件

1. 引入 `<dwg.h>` �?`<dwg_api.h>`，链�?`libredwg`
