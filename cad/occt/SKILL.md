...
- **几何与拓 -- ?*：`gp_*` 几何基元、`Geom_*` 解析曲线/曲面、`TopoDS_Shape`（Vertex/Edge/Wire/Face/Shell/Solid/Compound -- ?
- **建模算法**：拉 -- ?旋转/扫掠/放样/圆角/倒角/抽壳/布尔（BOPAlgo -- ?
- **STEP / IGES / STL / OBJ / glTF / VRML / BREP** 数据交换
- **显示**：基 -- ?OpenGL（OCAF + AIS 应用框架 -- ?
...
# 源码：CMake 构建（OCCT 8.0.0 起要 -- ?C++17、CMake  -- ?3.10；VTK 默认关闭，需 -DUSE_VTK=ON 开启）
```

> **版本提示 -- ?* 当前稳定版为 **OCCT 8.0.0** -- ?026-05），是多年来最大改动：`Standard_Failure` 改为继承 `std::exception`；`Raise()`/`Throw()`/`Instance()` 静态助手被移除（改 -- ?`throw` 关键字）；`Standard_Mutex` 等退役，改用 `std::mutex`/`std::lock_guard`。从 7.x 升级需留意这些破坏性变更 -- ?
...
BRepMesh_IncrementalMesh(shape, 0.1);   // 网格化容 -- ?
...
### 工作流一：从草图 -- ?STEP 的完整零件建模（C++ -- ?

1. 使用 `BRepBuilderAPI_MakePolygon` 构建闭合多段 -- ? -- ?`BRepBuilderAPI_MakeFace` 生成 -- ?
2. `BRepPrimAPI_MakePrism(face, gp_Vec(0,0,h))` 将面拉伸为实 -- ?
3. 对实体边应用 `BRepFilletAPI_MakeFillet` 添加圆角
4. 使用 `BRepAlgoAPI_Cut` 等布尔操作减出孔/槽特 -- ?
5. `STEPControl_Writer` 导出 -- ?STEP 用于下游 CAM/装配
6. 若需网格输出，`BRepMesh_IncrementalMesh(shape, tolerance)`  -- ?`StlAPI_Writer` 输出 STL

### 工作流二：导 -- ?STEP 并提取几何信 -- ?

1. `STEPControl_Reader` 读取 STEP 文件  -- ?`TransferRoots()`  -- ?`OneShape()` 获取根形 -- ?
...
1. **小数容差一 -- ?*：使 -- ?`BRep_Builder.UpdateVertex(_, tol)`，避免布尔失 -- ?
2. **`ShapeFix_Shape`**：导入外 -- ?STEP 后修 -- ?
3. **`BRepBuilderAPI_Sewing`**：缝合多张面 -- ?Shell
4. **使用 BOPAlgo_BOP** 替代 -- ?BRepAlgoAPI 处理多输入并行布 -- ?
...
| 问题 | 解决 |
|------|------|
| 布尔运算失败 | `ShapeFix_Shape` 修复输入；调整容 -- ?|
| STEP 中文乱码 | OCCT 7.6+ 设置 `STEPControl_Reader::SetReadProgress`  -- ?UTF-8 |
| 拷贝 Shape 后位置错 -- ?| 注意 `TopoDS_Shape` 是引用，深拷贝用 `BRepBuilderAPI_Copy` |
| 显示无图 | AIS 上下文未启动 OpenGL；检 -- ?`OCC.Display.SimpleGui` |

---

## AI 使用建议

- **推荐工作流模 -- ?*：AI 助手应根据语言偏好选择 OCCT 接口——C++ 直接使用原生 API，Python 使用 PythonOCC，Web 使用 OCCT.js。建模流程遵循「构造拓 -- ? -- ?布尔/修饰  -- ?数据交换」的工业模式，STEP 导入后务必用 `ShapeFix_Shape` 修复 -- ?
- **关键注意事项**：① 布尔运算失败时用 `ShapeFix_Shape` 修复输入并统一容差（`Precision::Confusion()`）； -- ?`TopoDS_Shape` 是引用语义，深拷贝用 `BRepBuilderAPI_Copy`；③ 多线程网格化 -- ?`OSD_Parallel` 启用，布尔运算非线程安全；④ 外部 STEP/IGES 导入后建议走修复流程 -- ?
...
- **cadquery**  -- ?基于 OCCT  -- ?Python 参数 -- ?CAD 库：[../cadquery/SKILL.md](../cadquery/SKILL.md)
- **chili3d**  -- ?Web CAD，使 -- ?OCCT.js（WebAssembly）：[../chili3d/SKILL.md](../chili3d/SKILL.md)
- **solvespace**  -- ?轻量约束求解 -- ?CAD（互补性工具）：[../solvespace/SKILL.md](../solvespace/SKILL.md)
