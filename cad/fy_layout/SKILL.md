...
FY_Layout 是飞扬集成设计平台（LightCAD）的 **场地布置二次开发插件示�?*，面向建筑工程设计行业，提供二维绘制与三维建模的完整插件示例源代码。该项目演示了如何基于飞扬平�?SDK 进行 C# 插件开发，涵盖元素定义、命令注册�?D/3D Action 编写及形�?实体 Provider 实现等核心流程�?

项目包含两个核心模块�?

| 模块 | 项目�?| 说明 |
|------|--------|------|
| 插件主体 | `QdLayout` | 场布插件——元素类型定义、命令注册、UI 菜单�?D/3D Action 实现 |
| 形状提供�?| `QdLayoutProvider` | 元素的二维形状（Shape）和三维实体（Solid）生成逻辑 |
...
- UI 框架：WinForms
- 图形引擎：SkiaSharp�?D）、ThreeJs4Net / OpenTK�?D�?
- 数据格式：兼�?DWG/DXF
...
- .NET 8.0 SDK
- 飞扬主程序（`飞扬主程�?lightcad.EXE`，项目仓库内已包含）
...
飞扬最新组件和启动程序�?`飞扬主程�?lightcad.EXE`。编译后使用 VS2022 **附加进程** 的方式进行断点调试�?
...
�?  ├── LcCurveChangeLoop.cs      # 曲线转换为闭合环工具�?
�?  ├── Barrier/                  # 防护栏杆
�?  ├── Berm/                     # 出土道路
�?  ├── Earthwork/                # 土方回填
�?  ├── Equipment/                # 场布设备
...
| DLL | 命名空间 | 用�?|
|-----|---------|------|
| `LightCAD.Core.dll` | `LightCAD.Core` | �?核心——元素类�?`ElementType`、文�?`LcDocument`、运行时 `LcRuntime`、组件定义等 |
| `LightCAD.Drawing.dll` | `LightCAD.Drawing` | 绘图文档——图层、图纸、绘制管�?|
| `LightCAD.Drawing.Actions.dll` | `LightCAD.Drawing.Actions` | 绘图交互动作基类——`IDocumentEditor`、点拾取、线拾取�?|
| `LightCAD.MathLib.dll` | `LightCAD.MathLib` | 数学库——`Vector2`、`Vector3`、`Line2d`、`Arc2d`、`Polyline2d`、`Color` �?|
| `LightCAD.Model.dll` | `LightCAD.Model` | 模型管理 |
| `LightCAD.Runtime.dll` | `LightCAD.Runtime` | 运行时框架——`CommandClass`、`CommandMethod`、`TabItem`、`TabButton` 等特性和 UI �?|
| `LightCAD.RenderUtils.dll` | `LightCAD.RenderUtils` | 渲染工具 |
| `ThreeJs4Net.dll` | `ThreeJs4Net` | 三维引擎——`Shape`、`Solid3d`、`Surface3d`、`GeometryData` �?|
| `OpenTK.dll` | `OpenTK` | OpenGL 绑定 |
| `Newtonsoft.Json.dll` | `Newtonsoft.Json` | JSON 序列�?|
...
使用 `[CommandClass]` �?`[CommandMethod]` 特性注册绘图命令：
            ...
            Buttons = new List<TabButton>
            {
                new TabButton
                {
                    ...
                    DropDowns = new List<TabButton>
                    {
                        ...
                        new TabButton { Name = "LawnChange", Text = "转换多段�?, IsCommand = true },
                    }
                },
                // ... 更多按钮
            }
...
| 元素名称 | ElementType �?| 实体�?| 2D Action | 3D Action | 说明 |
|---------|---------------|--------|-----------|-----------|------|
| 草坪 | `Lawn` | `QdLawn` | `LawnAction` | `Lawn3dAction` | 支持任意绘制、矩形绘制、多段线转换 |
| 围栏 | `Fence` | `QdFence` | `FenceAction` | `Fence3dAction` | 围墙绘制 |
| 拟建建筑 | `PlanBuild` | `QdPlanBuild` | `PlanBuildAction` | `PlanBuild3dAction` | 支持多段线拾取创�?|
...
| 防护栏杆 | `Barrier` | `QdBarrier` | `BarrierAction` | `Barrier3dAction` | 线性防护构�?|
...
| 开门边�?| `OpenLine` | `QdOpenLine` | `OpenLineAction` | �?| 出入口边�?|
...
| `PickLinePlanBuild` | PLPB | `PlanBuildAction` | `ExecPickLineCreatePlanBuild` | 拾取线创建拟建建�?|
...
| `OpenOuterLine` | OPL | `OpenLineAction` | `ExecCreateOpenLine` | 绘制开门边�?|
...
| `DrawOrAdjust` | DRAD | `DrawOrAdjustAction` | `ExecCreate` | 绘制或调�?|
...
| 文件 | 说明 |
|------|------|
| `Qd{Element}.cs` | 元素实体类，继承�?LightCAD 基类，定义元素属性和序列�?|
| `Qd{Element}Def.cs` | 元素定义�?|
...
以场地（Site）Provider 为例，展示如何注册一个二维形�?Provider�?
...
| 场景 | 涉及�?模式 | 说明 |
|------|-----------|------|
| 添加新的场布元素类型 | `LayoutElementType` + `ILcPlugin.Loaded()` | 定义新的 `ElementType`（含唯一 GUID），在插�?`Loaded()` 中注�?|
| 注册新命�?| `[CommandClass]` + `[CommandMethod]` | 在命令类中添加新方法，指�?Name �?ShortCuts |
| 实现 2D 绘制交互 | `{Element}Action` + `IDocumentEditor` | 实现 `ExecCreatePoly` / `ExecCreateRec` / `ExecCreate` 方法 |
| 实现 3D 建模 | `{Element}3dAction` + Provider | �?Provider 中用 `Curve2dGroupCollection` �?`Solid3dCollection` 生成模型 |
| 添加属性设置面�?| `{Element}Set` (WinForms Form) | 创建 WinForms 面板供用户编辑元素参�?|
| 自定�?UI 菜单按钮 | `TabItem` / `TabButtonGroup` / `TabButton` | �?`LayoutPlugin` 中添加新的按钮定�?|
| 注册形状/实体 Provider | `IDllProviderImporter` + `ConvertToProviders` | �?Provider 项目中实�?`CreateShape` �?`CreateSolid` 委托 |
| 曲线转换为闭合环 | `LcCurveChangeLoop.CheckLoops()` | 将多条线�?弧线拼接为闭合多段线 |
...
| �?| 命名空间 | 说明 |
|----|---------|------|
| `Vector2` | `LightCAD.MathLib` | 二维向量，支持加减、缩放、旋转、距离计�?|
...
| `Circle2d` | `LightCAD.MathLib` | 二维�?|
| `Plane` | `LightCAD.MathLib` | 平面（法向量定义�?|
| `Curve2dGroup` | `LightCAD.MathLib` | 二维曲线�?|
...
1. **GUID 唯一�?*：每�?`ElementType` �?Provider �?UUID 必须全局唯一，使�?`Guid.ParseExact(..., "B")` 格式�?
2. **多段线方�?*：闭合多段线需检查绕行方向，使用 `ShapeUtils.isClockWise()` 判断并在必要时调�?`Reverse()`�?
3. **Clone 习惯**：几何对象（`Vector2`、`Line2d`、`Polyline2d` 等）在变换前务必 `Clone()`，避免修改原始数据�?
4. **Provider 项目独立**：`QdLayoutProvider` 输出�?`Build/Providers` 目录，与插件主体分离部署�?
5. **调试方式**：编译后通过 VS2022 附加进程�?`lightcad.EXE` 进行断点调试�?
6. **UI 注册时机**：`TabItem` 应在 `Completed()` 中通过 `AppRuntime.UISystem.AddInitTabItems()` 注册，不要在 `Loaded()` 中操�?UI�?

---

## AI 使用建议

- **推荐工作流模�?*：AI 助手开发飞扬插件应遵循「ElementType 定义 �?ILcPlugin 注册 �?Command 声明 �?Action 实现 �?Provider 生成」的五步模式。新元素先参�?`Lawn`/`Fence` 的完整实现作为模板�?
- **关键注意事项**：① 每个 `ElementType` �?Provider UUID 必须全局唯一，使�?`Guid.ParseExact(..., "B")` 格式；② 几何对象变换前必�?`Clone()`，避免修改原始数据；�?`TabItem` �?`Completed()` 中注册，不可�?`Loaded()` 中操�?UI；④ Provider 项目与插件主体分离部署，输出�?`Build/Providers`�?
- **常用代码模式**：插件骨�?= `ILcPlugin.Loaded()` 注册元素类型�?Action + `Completed()` 注册 UI 菜单�?D Action 模式 = `ExecCreatePoly`（交互式点拾取）/ `ExecCreateRec`（矩形）/ `ExecCreate`（多段线转换）。Provider 模式 = `ConvertToProviders` 注册 Shape �?`ConvertToProvider` 注册 Solid �?几何生成方法返回 `Curve2dGroupCollection`/`Solid3dCollection`�?
...
- **ifoxcad** �?AutoCAD .NET 二次开发框架（类似�?CAD 插件开发模式）：[../ifoxcad/SKILL.md](../ifoxcad/SKILL.md)
...
- **飞扬集成设计平台�?* 面向建筑设计行业的开�?BIM 正向设计软件
- **技术服务：** 微信 qishou003