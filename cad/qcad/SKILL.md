...
>
> **许可证：** GPL-3.0（社区版）；Pro 版商业许 -- ?

## 概述

QCAD 由两部分组成 -- ?

- **QCAD Community Edition**：开源核心，支持 DXF（自带）+ DWG（需 -- ?Teigha/ODA File Converter  -- ?LibreDWG -- ?
- **QCAD/CAM  -- ?QCAD Pro**：商业插件（数控、PDF、SVG 详细输出等）

核心能力 -- ?

- 2D 绘图：线/ -- ? -- ?多段 -- ?样条/标注/填充/文本
- 图层、块、视图、布局、打 -- ?
- DXF/DWG/PDF/SVG 输入输出
- ECMAScript（QtScript / V8）脚本扩 -- ?
- 命令行：`qcad`、`dwg2dxf`、`dxf2pdf`、`dxf2bmp`  -- ?
...
# 命令 -- ?
qcad             # 启动 GUI
qcad -no-gui -autostart script.js   # 无界面运行脚 -- ?
```

 -- ?ODA File Converter（免费）放在 PATH 中以支持 DWG -- ?
...
# DWG  -- ?DXF（需 ODA File Converter  -- ?LibreDWG -- ?
...
| 选择 -- ?| `getDocumentInterface().getStorage().queryAllSelectedEntities()` |
|  -- ?| `RBlockReferenceEntity` + `RBlock` |
...
2. 使用 `RDocument` + `RMemoryStorage` 处理 -- ?UI 转换
3. `RAddObjectsOperation.addObject(obj, false)` 不立即更新空间索 -- ?
4. 大量实体批量提交后再 `applyOperation`
5. 禁用快照（snap）与轨迹（tracking）以加速绘 -- ?
...
| 字体缺失 | 复制 .shx/.ttf  -- ?`fonts/` 目录或安装到系统 |
| 中文显示 -- ?| 设置图层使用支持 CJK 的字体（ -- ?simhei -- ?|
| 脚本路径 -- ?| 使用绝对路径 -- ?`documentInterface.getDocument()` |

---

## AI 使用建议

- **推荐工作流模 -- ?*：AI 助手应根据场景选择 QCAD 接口——交互式操作使用 GUI 命令，批处理使用 `qcad -no-gui -autostart script.js` 无界面运行。ECMAScript 脚本遵循「获取文 -- ? -- ?构建操作（RAddObjectsOperation 等） -- ?applyOperation」的模式。DWG 文件需 ODA File Converter 支持 -- ?
- **关键注意事项**：① DWG 支持依赖 ODA File Converter（免费），需 -- ?Preferences 中配置路径； -- ?脚本需 `include("scripts/EAction.js")` 继承 EAction；③ 批量操作使用 `RAddObjectsOperation.addObject(obj, false)` 不立即更新空间索引，最后统一 `applyOperation`；④ 中文字体使用 CJK 支持 -- ?TTF（如 simhei） -- ?
...
1. 创建 `.js` 脚本文件，`include("scripts/EAction.js")` 引入基础 -- ?
2. 继承 `EAction`，在 `beginEvent()` 中通过 `this.getDocument()` 获取文档
3. 构建 `RLineEntity`/`RCircleEntity` 等实体对 -- ?
4. 使用 `RAddObjectsOperation` 批量添加实体
5. `doc.applyOperation(op)` 提交操作
6. 通过 `Misc  -- ?Run script` 运行，或 `qcad -no-gui -autostart script.js` 无界面执 -- ?

### 工作流二：命令行批量格式转换

1. `dxf2pdf -o out.pdf input.dxf` DXF  -- ?PDF
2. `dxf2svg -o out.svg input.dxf` DXF  -- ?SVG
3. `dxf2bmp -a -o out.png input.dxf` DXF  -- ?PNG 位图
4. `dwg2dxf -outversion=2018 input.dwg` DWG  -- ?DXF（需 ODA File Converter -- ?
