...
支持文件：FCStd、STEP、IGES、BREP、STL、OBJ、PLY、DXF、DWG、SVG、IFC、PDF（导入）、AMF�?MF�?
...
最新稳�?FreeCAD 1.0.x / 1.1.x 起合并了大量长期分支（如 PartDesign Toponaming 修复）�?
...
2. **新建 Body** �?进入 Sketcher，绘制带约束�?2D 草图
...
- Arch / BIM 工作台支持墙、梁、柱、楼板、屋顶、门窗、设�?
- 支持 IFC2x3 / IFC4 导入导出（基�?IfcOpenShell�?
- �?Blender BIM、IfcConvert 协同
...
1. **关闭工具栏图标动�?*与不必要的工作台
2. **大装�?*：使�?Lightweight Part / Linked Document
3. **Recompute** 仅在必要时调用，使用 `doc.recompute(returnStringList=True)` 检查错�?
...
| 问题 | 解决 |
|------|------|
| Toponaming 在编辑后特征丢失 | 升级�?FreeCAD 1.0.x / 1.1.x（含拓扑命名修复�?|
| 中文文件名编�?| 使用 UTF-8；Windows 下避�?GBK 路径 |
| 导出 STEP 缺少颜色 | �?STEP AP242 或在 ImportSettings 启用颜色 |
| Sketcher 求解失败 | 删除冗余约束、检�?DoF |
| Mesh �?Solid 不闭�?| �?Part �?Convert to solid 前先 Mesh Repair |

---

## AI 使用建议

- **推荐工作流模�?*：AI 助手应根据场景选择工作台——机械零件用 PartDesign（草�?特征），简单几何用 Part（CSG），建筑�?Arch/BIM。脚本自动化优先使用 `FreeCAD Python Console` �?`freecadcmd` 无头模式�?
- **关键注意事项**：① Sketcher 草图需完全约束（黑色），欠约束（蓝色）在后续编辑中可能偏移；② Python 操作几何对象后需调用 `doc.recompute()` 才能更新显示与关联；�?导出 STEP 前确认模型无错误（`Part �?Check Geometry`）；�?Toponaming 问题�?FreeCAD 1.0.x / 1.1.x 版基本解决，旧版注意避免引用�?边编号�?
- **常用代码模式**：`doc.addObject("Part::Box", ...)` 创建参数化体 �?设置属�?�?`doc.recompute()` �?`Part.export([shape], "out.step")`。对于脚本化建模，也可直接用 `Part.make*` 系列函数构造几何�?
...
- **cadquery** �?Python 参数�?CAD 库（同基�?OCCT，更适合纯脚本）：[../cadquery/SKILL.md](../cadquery/SKILL.md)
- **openscad** �?声明�?CSG 建模（与 FreeCAD 互补，适合编程型用户）：[../openscad/SKILL.md](../openscad/SKILL.md)
