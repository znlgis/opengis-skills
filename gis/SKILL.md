---
name: gis-skills
description: "Use when processing geospatial data, publishing map services, querying spatial databases, performing geometry operations, or building web map applications. Index of 23 skills: GDAL, GeoServer, QGIS, PostGIS, JTS, GeoPandas, Shapely, CesiumJS, OpenLayers, NetTopologySuite and more."
tags:
  - gis
  - geospatial
  - mapping
  - spatial-analysis
  - vector
  - raster
  - server
  - webmapping
---

> **父级入口 -- ?* [../SKILL.md](../SKILL.md)  -- ?全仓 66 技能总索 -- ?

## 概述

本分类涵 -- ?**23  -- ?GIS 开源项 -- ?*的技能文件，覆盖从底层数据处理到上层地图服务的完整链路：

```
数据 -- ? -- ?命令行处 -- ? -- ?编程分析  -- ?空间数据 -- ? -- ?地图服务  -- ?前端可视 -- ?
 (GDAL)   (GDAL CLI)   (JTS/PyQGIS)  (PostGIS)  (GeoServer) (OpenLayers/Cesium)
```

### 何时加载此索引？

- 用户问题涉及「地图」「空间数据」「坐标」「GIS」等关键 -- ?
- 不确定具体该 -- ?GDAL、QGIS 还是 PostGIS
- 需要了 -- ?GIS 领域有哪些可选开源工 -- ?

---

## 技能列 -- ?

### 🧰 数据处理与命令行

| 技 -- ?| 简 -- ?| 关键标签 |
|------|------|---------|
| [gdal](./gdal/SKILL.md) | GDAL 命令行：栅格/矢量处理事实标准 | `cli` `raster` `vector` `conversion` |
| [gdal-api](./gdal-api/SKILL.md) | GDAL/OGR 编程 API（C/C++/Python/.NET -- ?| `api` `python` `cpp` `dotnet` |
| [qgis-process](./qgis-process/SKILL.md) | QGIS 命令行批处理 | `cli` `processing` `headless` |
| [geopipe-agent](./geopipe-agent/SKILL.md) | AI 原生 GIS 数据流水线（YAML 驱动 -- ?| `agent` `pipeline` `yaml` `qc` |

### 🗄 -- ?空间数据 -- ?

| 技 -- ?| 简 -- ?| 关键标签 |
|------|------|---------|
| [postgis](./postgis/SKILL.md) | PostgreSQL 空间数据库扩 -- ?| `database` `sql` `spatial` `postgresql` |

### 🔧 几何运算 -- ?

| 技 -- ?| 简 -- ?| 关键标签 |
|------|------|---------|
| [jts](./jts/SKILL.md) | Java Topology Suite 几何引擎 | `java` `geometry` `spatial` |
| [geometry-api-java](./geometry-api-java/SKILL.md) | Esri Geometry API for Java | `java` `esri` `geometry` |
| [geometry-api-net](./geometry-api-net/SKILL.md) | Esri Geometry API for .NET | `dotnet` `esri` `geometry` |
| [shapely](./shapely/SKILL.md) | Python 几何对象与运 -- ?| `python` `geometry` `analysis` |
| [geopandas](./geopandas/SKILL.md) | Python 矢量空间数据处理 | `python` `pandas` `vector` |

### 🌐 地图服务 -- ?

| 技 -- ?| 简 -- ?| 关键标签 |
|------|------|---------|
| [geoserver](./geoserver/SKILL.md) | 开源地图服务器（WMS/WFS/WMTS/WCS -- ?| `server` `wms` `wfs` `ogc` |
| [geoserver-rest-api](./geoserver-rest-api/SKILL.md) | GeoServer REST API 自动化管 -- ?| `rest` `automation` `publish` |
| [geoserver-cloud](./geoserver-cloud/SKILL.md) | GeoServer 云原生微服务架构 | `cloud` `kubernetes` `docker` |

### 🖥 -- ?QGIS 生 -- ?

| 技 -- ?| 简 -- ?| 关键标签 |
|------|------|---------|
| [pyqgis](./pyqgis/SKILL.md) | QGIS Python 二次开 -- ?| `python` `qgis` `plugin` |

### 🗺 -- ?Web 地图可视 -- ?

| 技 -- ?| 简 -- ?| 关键标签 |
|------|------|---------|
| [cesiumjs](./cesiumjs/SKILL.md) | 高性能 3D 地球可视 -- ?| `javascript` `3d` `webgl` |
| [openlayers](./openlayers/SKILL.md) | 高性能 Web 2D 地图 -- ?| `javascript` `webmapping` `2d` |

### 📦 .NET GIS 组件

| 技 -- ?| 简 -- ?| 关键标签 |
|------|------|---------|
| [nettopologysuite](./nettopologysuite/SKILL.md) | JTS  -- ?.NET 移植 | `dotnet` `geometry` |
| [geometry-api-net](./geometry-api-net/SKILL.md) | Esri Geometry API for .NET | `dotnet` `esri` `geometry` |
| [sharpmap](./sharpmap/SKILL.md) | .NET WinForms/Web 地图渲染 -- ?| `dotnet` `legacy` `winforms` |
| [mapsui](./mapsui/SKILL.md) | .NET 跨平台地图控 -- ?| `dotnet` `crossplatform` `maui` |

### 🧩 综合/工具 -- ?

| 技 -- ?| 简 -- ?| 关键标签 |
|------|------|---------|
| [opengis-all](./opengis-all/SKILL.md) | **一站式 GIS 全流程索 -- ?* | `entrypoint` `workflow` `fullstack` |
| [geotools](./geotools/SKILL.md) | Java GIS 工具 -- ?| `java` `geotools` `feature` |
| [opengis-utils-for-java](./opengis-utils-for-java/SKILL.md) | OpenGIS Java 实用工具 -- ?| `java` `utils` `toolkit` |
| [opengis-utils-for-net](./opengis-utils-for-net/SKILL.md) | OpenGIS .NET 实用工具 -- ?| `dotnet` `utils` `toolkit` |

---

## 快速导 -- ?

| 用户需 -- ?| 推荐加载 |
|---------|---------|
| "Shapefile  -- ?GeoJSON" | `gdal/SKILL.md` |
| "Python 空间分析" | `geopandas/SKILL.md` + `shapely/SKILL.md` |
| "发布 WMS 地图服务" | `geoserver/SKILL.md` + `geoserver-rest-api/SKILL.md` |
| "QGIS 批处 -- ? | `qgis-process/SKILL.md` |
| "Java 几何运算" | `jts/SKILL.md` |
| ".NET 地图控件" | `mapsui/SKILL.md` |
| "PostgreSQL 空间查询" | `postgis/SKILL.md` |
| "前端 3D 地球" | `cesiumjs/SKILL.md` |
| "前端 2D 地图" | `openlayers/SKILL.md` |
| "AI 驱动 -- ?GIS 流水 -- ? | `geopipe-agent/SKILL.md` |
| "完整 GIS 处理链路" | `opengis-all/SKILL.md` |

---

## 相关分类

- **[cad/](../cad/SKILL.md)**  -- ?CAD 参数化建模、几何运算、BIM/PCB
- **[csharp/](../csharp/SKILL.md)**  -- ?.NET 框架、ORM、Office 操作
- **[ai/](../ai/SKILL.md)**  -- ?LLM 应用、Agent 框架
- **[3d/](../3d/SKILL.md)**  -- ?3D 高斯泼溅 -- ?Web 三维可视 -- ?
- **[iot/](../iot/SKILL.md)**  -- ?物联网与嵌入 -- ?
