...
JTS Topology Suite（简�?JTS）是 LocationTech 项目组下的开�?Java 二维矢量几何库，�?GeoTools、GeoServer 等众多开�?GIS 项目的几何计算核心。它提供�?
...
```xml
<properties>
    <jts.version><!-- 请查�?Maven Central 获取最新版：https://central.sonatype.com/artifact/org.locationtech.jts/jts-core --></jts.version>
</properties>
...
| 模块 | artifactId | 用�?|
|---|---|---|
| jts-core | `jts-core` | �?核心——几何模型、算法、空间操作、索�?|
...
| `LineString` | `org.locationtech.jts.geom` | 线串�? 维） |
| `LinearRing` | `org.locationtech.jts.geom` | 闭合线环（用于构�?Polygon�?|
...
| `PreparedGeometry` | `org.locationtech.jts.geom.prep` | 预处理几何，加速重复空间关系判�?|
| `PreparedGeometryFactory` | `org.locationtech.jts.geom.prep` | 创建 PreparedGeometry 的工�?|
...
| `TopologyPreservingSimplifier` | `org.locationtech.jts.simplify` | 保持拓扑的几何简�?|
| `DouglasPeuckerSimplifier` | `org.locationtech.jts.simplify` | Douglas-Peucker 几何简�?|
| `IsValidOp` | `org.locationtech.jts.operation.valid` | 几何有效性检�?|
| `LengthIndexedLine` | `org.locationtech.jts.linearref` | 按长度进行线性参�?|
| `LocationIndexedLine` | `org.locationtech.jts.linearref` | 按位置进行线性参�?|
...
Geometry bufferFlat = geometry.buffer(10.0, 8, BufferOp.CAP_FLAT);  // 平头�?
...
当需要对同一个几何反复进行空间关系判断时（如判断大量点是否在某面内），使�?`PreparedGeometry` 可显著提升性能�?
...
| 计算两个几何的距�?| `Geometry.distance()` |
| 缓冲区分�?| `Geometry.buffer()` �?`BufferOp` |
...
| 线性参�?/ 沿线定位 | `LengthIndexedLine` / `LocationIndexedLine` |
| 凸包计算 | `Geometry.convexHull()` |
| 几何仿射变换 | `AffineTransformation` |
| 数据�?WKB 交互 | `WKBReader.hexToBytes()` / `WKBWriter.toHex()` |

---

## 常见注意事项

1. **使用 GeometryFactory 创建几何**：不要直�?`new Point()`，始终通过 `GeometryFactory` 的工厂方法创建几何对象�?
2. **面必须闭�?*：Polygon 的外环和内环坐标数组的首尾坐标必须相同�?
3. **坐标顺序**：JTS 使用 `(x, y)` �?`(经度, 纬度)` 的顺序，注意与某�?GIS 系统�?`(纬度, 经度)` 区分�?
4. **几何有效�?*：从外部导入的几何数据应使用 `geometry.isValid()` 检查有效性，无效几何可用 `GeometryFixer.fix()` 修复�?
5. **SRID 不参与计�?*：JTS �?SRID 仅作为元数据标记，不影响空间运算；JTS 所有计算都在笛卡尔平面上进行，不处理投影�?
6. **性能优化**：批量空间查询使�?`STRtree`；重复空间关系判断使�?`PreparedGeometry`；大量几何合并使�?`UnaryUnionOp`�?
7. **线程安全**：`GeometryFactory` 是线程安全的；`Geometry` 对象本身不可变，可安全共享；�?Reader/Writer 实例非线程安全，需为每个线程创建独立实例�?
8. **包名迁移**：JTS 1.15+ 包名�?`com.vividsolutions.jts` 迁移�?`org.locationtech.jts`，注意旧代码升级�?
...
1. **创建几何对象**：始终通过 `GeometryFactory` 工厂方法创建，不要直�?`new`
2. **格式解析**：从 WKT/WKB/GeoJSON 读入外部数据，使用对应的 Reader
3. **空间运算**：用 `PreparedGeometry` 加速批�?contains/intersects 判断
4. **批量查询**：用 `STRtree` 建空间索引后查询
5. **结果导出**：用 Writer 输出�?WKT/WKB/GeoJSON

### 关键注意事项

- **坐标顺序**：JTS 使用 `(x, y)` �?`(经度, 纬度)`，注意与部分 GIS 系统�?`(y, x)` 区分
- **SRID 不参与计�?*：JTS 所有计算在笛卡尔平面进行，不处理地球曲�?
- **OverlayNG 优先**：叠加运算优先使�?`OverlayNGRobust.overlay()`，比传统方法更鲁�?
- **线程安全**：`GeometryFactory` �?`Geometry` 线程安全；Reader/Writer 非线程安�?
- **有效性检�?*：外部导入的几何�?`isValid()` 检查后�?`GeometryFixer.fix()` 修复

## 相关技�?

- **nettopologysuite** �?JTS �?.NET 移植：[../nettopologysuite/SKILL.md](../nettopologysuite/SKILL.md)
