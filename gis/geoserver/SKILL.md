...
>  -- ?SKILL 关注 GeoServer 服务端运维与配置；REST 自动化管理资源请配合 `gis/geoserver-rest-api` -- ?
...
- **数据 -- ?*：PostGIS、Shapefile、GeoPackage、Oracle、SQL Server、ArcSDE、GeoTIFF、ImageMosaic、JP2、NetCDF
- **安全**：基于角色的访问控制（RBAC -- ?
- **管理**：Web UI（端 -- ?8080 -- ?+ REST API
...
2. 添加 Store： -- ?PostGIS，填 host/db/user/pass
...
- **ImageMosaic**：目录中放多 -- ?GeoTIFF（同投影），创建 Mosaic store；推荐配 -- ?`gdaladdo` 建金字塔
...
| 问题 | 解决 |
|------|------|
| 启动 -- ?/ OOM | 增加 `-Xmx`，关 -- ?demo 数据 |
| Shapefile 中文乱码 | Store  -- ?`charset=UTF-8`  -- ?`GBK` |
...
1. **部署 GeoServer**：优先使 -- ?Docker 镜像 `docker.osgeo.org/geoserver:3.0.0`
2. **创建 Workspace**：为每个项目创建独立工作空间实现命名隔离
3. **添加数据存储**：根据数据类型选择 PostGIS（矢量）、Shapefile（文件矢量）、GeoTIFF（栅格）
4. **发布图层**：设 -- ?SRS（坐标参考系）、计 -- ?Native Bounds、配置样 -- ?
5. **验证服务**：通过 Layer Preview 或直接构 -- ?WMS/WFS 请求测试
6. **优化性能**：启 -- ?GeoWebCache 预切片、PostGIS 建空间索引、调 -- ?JVM 参数

### 关键注意事项

- **数据源编 -- ?*：Shapefile 中文乱码时在 Store 设置 `charset=UTF-8`  -- ?`GBK`
- **Native Bounds**：必须正确计算图层的原始范围，否 -- ?WMS 请求可能返回空白
- **级联删除**：删 -- ?Workspace/Store 时添 -- ?`?recurse=true` 参数
- **JVM 调优**：生产环境建 -- ?`-Xms2g -Xmx4g -XX:+UseG1GC`
- **CORS 配置**：跨域访问需 -- ?`web.xml` 中配 -- ?CORS 过滤 -- ?
- **SLD 样式**：使 -- ?SLD Cookbook 快速参考常用样式模 -- ?
