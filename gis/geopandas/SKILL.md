...
GeoPandas  -- ?Python 地理空间矢量数据处理的核心库，在 pandas DataFrame 基础上扩展了几何列支持。核心类 `GeoDataFrame`（继 -- ?`pandas.DataFrame`）和 `GeoSeries`（继 -- ?`pandas.Series`）通过 Shapely 提供几何运算、通过 pyproj 提供坐标系管理、通过 pyogrio/fiona 提供文件 IO -- ?
...
| `shapely` | >= 2.0.0 | 几何对象与运 -- ?(GEOS) |
| `pyproj` | >= 3.5.0 | CRS 处理与坐标变 -- ?|
| `pyogrio` | >= 0.7.2 | 默认文件 IO 引擎 (GDAL/OGR) |
| `numpy` | >= 1.24 | 数组运算 |

| 可选依 -- ?| 用 -- ?|
|----------|------|
| `matplotlib` >= 3.5 | 静态地图绘 -- ?`.plot()` |
| `folium` | 交互式地 -- ?`.explore()` |
| `mapclassify` >= 2.7 | 分级设色分类方案 |
| `fiona` >= 1.8.21 | 备选文 -- ?IO 引擎 |
...
| 方法 | 说明 |
|------|------|
| `to_file(filename, driver=None, ...)` | Shapefile/GPKG/GeoJSON  -- ?|
| `to_parquet(path, compression="snappy", geometry_encoding="WKB", ...)` | GeoParquet |
| `to_feather(path, ...)` | Feather |
| `to_json(na="null", show_bbox=False, to_wgs84=False, ...)` | GeoJSON 字符 -- ?|
...
| `count_coordinates()` | Series[int] | 坐标对数 -- ?|
| `count_geometries()` | Series[int] | 多部件几何体部件 -- ?|
| `get_coordinates()` | DataFrame | 提取所有坐 -- ?|
...
引擎选项：`"pyogrio"`（默认） -- ?`"fiona"`。支持所 -- ?GDAL/OGR 驱动 -- ?
...
- `mask`: GeoDataFrame、GeoSeries -- ?Multi)Polygon  -- ?`(minx, miny, maxx, maxy)` 元组
...
需 -- ?`geopy`。默 -- ?provider: `"photon"`。返回含 `geometry`  -- ?`address` 列的 GeoDataFrame -- ?

### 其他工具

```python
gpd.tools.collect(x, multi=False)  # 收集几何体为多部 -- ?
...
`scheme` 参数需 -- ?`mapclassify`，支 -- ?`"quantiles"`, `"equal_interval"`, `"fisher_jenks"` 等分类方案 -- ?
...
```python
gdf = gdf.set_crs("EPSG:4326")          # 赋 -- ?CRS（数据本身不变）
gdf = gdf.to_crs("EPSG:3857")           # 投影变换（坐标重算）
gdf = gdf.to_crs(epsg=32650)            #  -- ?EPSG 代码投影
utm = gdf.estimate_utm_crs()            # 自动检 -- ?UTM 分区
```

**注意**：`set_crs()` 仅标 -- ?CRS 不转换坐标；`to_crs()` 实际执行坐标变换。面 -- ?距离计算前务必投影到适当的投影坐标系（如 UTM） -- ?
...
clipped = gpd.clip(gdf, (xmin, ymin, xmax, ymax))  # 矩形快速裁 -- ?
...
| 问题 | 原因 | 解决方案 |
|------|------|----------|
| 面积/距离 -- ?0 或极 -- ?| CRS 为地理坐标系 ( -- ? | `gdf.to_crs(gdf.estimate_utm_crs())` 后计 -- ?|
| `sjoin` 无结 -- ?| 两个 GDF  -- ?CRS 不一 -- ?| 统一 CRS：`gdf2 = gdf2.to_crs(gdf1.crs)` |
| `ValueError: GeoDataFrame does not support multiple columns using the geometry column name 'geometry'` | 多个列名 -- ?`geometry` | 重命名冲突列或使 -- ?`set_geometry()` 指定 |
| `to_file` 属性被截断 | Shapefile 字段名限 -- ?10 字符 | 使用 GeoPackage (`.gpkg`)  -- ?GeoJSON |
| `set_crs` vs `to_crs` 混淆 | `set_crs` 仅赋值不转换 | 已有 CRS 需要转换用 `to_crs()`；无 CRS 标记 -- ?`set_crs()` |
| Shapefile 不支持空 -- ?日期时间 | Shapefile 格式限制 | 使用 GeoPackage 格式 |
| `DriverError` 无法写入 | 驱动不支持或文件被占 -- ?| 检查驱动名拼写、关闭已打开的文 -- ?|
| `align=True` 导致意外 NaN | 二元操作默认按索引对 -- ?| 传入 `align=False` 或重置索 -- ?|

## AI 使用建议

### 推荐工作 -- ?

1. **读取数据**：使 -- ?`gpd.read_file()` 读取矢量文件，或 `gpd.read_parquet()` 读取 GeoParquet
2. **检 -- ?CRS**：始终先检 -- ?`gdf.crs`，需要在投影坐标系（非地理坐标系）下进行面积/距离计算
3. **空间分析**：使 -- ?`gpd.sjoin()` 进行空间连接，`gpd.overlay()` 进行叠加分析，`gpd.clip()` 进行裁剪
4. **可视 -- ?*：使 -- ?`.plot()` 快速静态出图，`.explore()` 生成交互式地 -- ?
5. **输出**：使 -- ?`gdf.to_file()`  -- ?`gdf.to_parquet()` 保存结果，优先使 -- ?GeoPackage  -- ?GeoParquet 格式

### 关键注意事项

- **`set_crs()` vs `to_crs()`**：`set_crs()` 仅标记坐标系不转换坐标，`to_crs()` 执行实际坐标变换——切勿混 -- ?
- **面积/距离计算前投 -- ?*：地理坐标系（EPSG:4326）下面积单位为度²无意义，应使 -- ?`gdf.to_crs(gdf.estimate_utm_crs())` 投影后再计算
- **空间操作前统一 CRS**：确保参 -- ?`sjoin`、`overlay`、`clip` 的所 -- ?GeoDataFrame 使用相同 CRS
- **优先使用 GeoParquet**：比 Shapefile/GeoJSON 读写 -- ?5-10 倍，且无字段名长度限 -- ?
- **向量化操 -- ?*：避免逐行循环，使 -- ?GeoSeries 的向量化方法（`buffer()`、`intersection()` 等）批量处理
- **空间索引自动构建**：`.sindex` 自动维护 STRtree，`query()`  -- ?`nearest()` 可大幅加速空间查 -- ?
...
- **GeoParquet**  -- ?Shapefile/GeoJSON 读写 -- ?5-10 倍，优先使用
- **空间索引** `.sindex` 自动构建 STRtree，`query()`  -- ?`nearest()` 可大幅加速空间查 -- ?
- **矩形裁剪** `clip_by_rect()`  -- ?`clip(gdf, (xmin,ymin,xmax,ymax))` 比多边形裁剪 -- ?
- **`pyogrio` 引擎**（默认） -- ?`fiona`  -- ?2-5  -- ?
- 大数据集 -- ?`read_file(rows=slice(0, 1000))` 先预 -- ?
