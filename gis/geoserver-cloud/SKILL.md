...
- **微服务架�?*：每�?OGC 服务为独�?Spring Boot 应用
- **配置同步**：`spring-cloud-bus` + RabbitMQ 实时同步
- **配置后端**：本地目�?/ **PgConfig（推荐）** / JDBCConfig
- **可观�?*：Actuator + Prometheus + Sleuth Tracing
- **容器友好**：官方镜像、Helm Chart、docker-compose
- **完全兼容社区�?*：相�?Web UI、SLD、数据存�?
...
任意修改 �?REST/web-ui 发�?`RemoteApplicationEvent` �?RabbitMQ �?所有副本订阅刷新本�?`Catalog`，秒级一致�?
...
1. WMS 多副�?+ Gateway 轮询；GWC 单独部署
2. PgConfig 数据库独�?+ 连接池调�?
3. RabbitMQ �?`delivery_limit` 避免风暴
...
| 问题 | 解决 |
|------|------|
| 配置不同�?| 检�?RabbitMQ + `spring.cloud.bus.enabled` |
| Web UI 改了 wms 没生�?| 检查事件订�?|
| 启动顺序错乱 | `depends_on` + 健康检查；K8s �?`initContainers` |
| OOM | 调整 `-Xmx`，按服务独立资源 |
...
1. **确定架构**：根据业务需求选择配置后端（生产推�?PgConfig�?
2. **部署基础设施**：先部署 RabbitMQ + PostgreSQL + Redis，再启动微服�?
3. **启动服务**：使�?docker-compose �?Helm Chart 一键部�?
4. **验证服务**：通过 Gateway (`localhost:9090/geoserver`) 访问 Web UI
5. **配置监控**：启�?Prometheus + Grafana 监控各微服务状�?
6. **弹性伸�?*：按负载独立扩缩 WMS/WFS 等服务的副本�?

### 关键注意事项

- **启动顺序**：先启动 discovery �?config �?数据�?RabbitMQ �?各业务服�?
- **配置同步**：确�?`spring.cloud.bus.enabled=true` �?RabbitMQ 可达
- **PgConfig 一致�?*：多副本共享同一�?PgConfig 数据库实现强一�?
- **JVM 独立调优**：每个微服务根据负载独立设置 `-Xmx`，避�?OOM
- **不部署不需要的服务**：如不使�?WPS/WCS，直接从 docker-compose 中移�?

## 典型工作�?

### 工作�?1：从零部署高可用 GeoServer Cloud
