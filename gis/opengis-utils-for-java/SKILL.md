...
metadata.setCoordinateSystemName("2000国家大地坐标�?);
metadata.setZoneDivision("3");
metadata.setProjectionType("高斯克吕�?);
metadata.setMeasureUnit("�?);
...
2. **读取数据**：使�?`OguLayerUtil.readLayer()` 统一入口读取各类矢量格式
3. **空间分析**：JTS Geometry 对象�?`GeometryUtil`�?0+ 方法），WKT 字符串用 `*Wkt` 后缀方法
4. **坐标转换**：使�?`CrsUtil.transform()` 进行 CGCS2000 系列坐标系变�?
5. **写出结果**：使�?`OguLayerUtil.writeLayer()` 写出为目标格�?

### 关键注意事项

- **始终使用 `OguLayerUtil` 作为主入�?*：自动处理引擎选择和格式路�?
- **几何�?WKT 存储**：`OguFeature.geometry` 存储 WKT 字符串；空间运算时转�?JTS Geometry
- **引擎选择**：`GEOTOOLS`（纯 Java，无原生依赖）vs `GDAL`（需本地库，支持 FileGDB�?
- **CGCS2000 坐标�?*：EPSG:4490 为默认地�?CRS，投�?CRS 范围 EPSG:4502-4554
- **Shapefile 字段名限 10 字符**：使�?`ShpUtil.formatFieldName()` 自动截断
- **资源释放**：`DataStore` �?OGR `DataSource` 必须�?finally 块中释放
...
9. **Attribute filtering** uses CQL expression syntax (e.g., `"population > 1000000 AND name LIKE '%�?'"`).
