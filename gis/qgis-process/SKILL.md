...
`qgis_process` �?QGIS 自带的命令行工具（QGIS Processing Executor），允许�?*不启�?QGIS 桌面应用**的情况下，直接从命令行运�?Processing 算法和模型（内置算法或由插件提供）。它适用于服务器端批量处理、CI/CD 自动化流水线、脚本化 GIS 工作流等场景�?
...
- 安装 QGIS 3.16+（`qgis_process` �?QGIS 3.16 开始可用）
...
|------|------|
| `--help` �?`-h` | 输出帮助信息 |
| `--version` �?`-v` | 输出 QGIS Process 相关的所有版本信�?|
| `--json` | �?JSON 格式输出结果（AI/脚本友好�?|
| `--verbose` | 输出详细日志 |
...
| `help <algorithm_id>` | 显示指定算法的帮助信�?|
| `run <algorithm_id> [-- PARAM=VALUE ...]` | 运行指定算法 |

> **注意�?* 只有�?`metadata.txt` 中声明了 `hasProcessingProvider=yes` 的已安装插件才能�?`qgis_process` 识别和加载�?
...
末尾�?`-` 表示�?STDIN 读取参数。JSON 必须包含 `"inputs"` 键：
...
| `native:symmetricaldifference` | 对称�?|
| `native:centroids` | 质心 |
| `native:convexhull` | 凸包 |
| `native:simplifygeometries` | 简化几�?|
| `native:reprojectlayer` | 重投�?|
| `native:mergevectorlayers` | 合并矢量图层 |
| `native:splitvectorlayer` | 拆分矢量图层 |
| `native:extractbyattribute` | 按属性提�?|
| `native:extractbylocation` | 按位置提�?|
| `native:joinattributesbylocation` | 按位置连接属�?|
| `native:fixgeometries` | 修复几何 |
| `native:countpointsinpolygon` | 多边形内点计�?|
| `native:voronoipolygons` | 泰森多边�?|
...
| `gdal:cliprasterbyextent` | 按范围裁剪栅�?|
| `gdal:cliprasterbymask` | 按掩膜裁剪栅�?|
| `gdal:merge` | 栅格合并 |
| `gdal:warpreproject` | 栅格重投�?|
| `gdal:contour` | 等值线提取 |
| `gdal:polygonize` | 栅格转矢�?|
| `gdal:rasterize` | 矢量转栅�?|
| `gdal:hillshade` | 山体阴影 |
| `gdal:slope` | 坡度分析 |
| `gdal:aspect` | 坡向分析 |
...
| `pdal:boundary` | 点云边界 |
| `pdal:density` | 点云密度 |
| `pdal:exportraster` | 点云转栅�?|
| `pdal:exportvector` | 点云转矢�?|
...
1. **发现算法**：`qgis_process list` �?获取所有可用算�?ID
2. **查看帮助**：`qgis_process help <algorithm_id> --json` �?获取参数定义�?JSON
3. **构造参�?*：根�?JSON 帮助信息构造参�?
4. **执行算法**：`qgis_process run <algorithm_id> --json -- PARAM=VALUE ...` 或通过 STDIN 传入 JSON
5. **解析结果**：解�?JSON 输出获取结果路径和日志信�?

### 关键注意事项

1. **始终使用 `--json` 选项**：JSON 输出结构化、易解析，是 AI 最友好的交互方式�?
2. **先查 help 再构造参�?*：每个算法的参数名和类型不同，先�?`help --json` 确认参数定义�?
3. **枚举类型传数字索�?*：枚举参数用数字（如 `0`、`1`）而非文字。help 输出�?`available_options` 提供了索引到含义的映射�?
4. **多值参数多次指�?*：如 `LAYERS=a.shp LAYERS=b.shp`（在 `--` 之后，参数不带前�?`--`）�?
5. **复杂参数�?STDIN JSON**：字典类型的参数值适合通过 STDIN JSON 方式传入�?
...
9. **取消正在运行的算�?*：使�?`CTRL+C` 可取消执行中的算法�?
10. **`--no-python` �?`--skip-loading-plugins` 可加速启�?*：如果不需要第三方插件提供的算法，使用这两个选项可显著缩短启动时间�?
...
- **`QgsProcessingExec`**：命令行执行器，负责解析命令和参数、加载插件、执行算�?
- **`ConsoleFeedback`**：继承自 `QgsProcessingFeedback`，处理控制台输出和进度显示，支持文本�?JSON 两种输出模式
...
| `listAlgorithms()` | 列出所�?Provider 和算�?|
