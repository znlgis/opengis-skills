...
GeoServer 是一个基�?Java 的开源地理空间数据服务器，实现了 OGC WMS、WFS、WCS、WMTS 等标准协议。GeoServer REST API 提供了完整的 RESTful 配置管理接口，可通过标准 HTTP 方法（GET / POST / PUT / DELETE）对 GeoServer 的各类资源进行编程式管理�?

REST API 的典型用途包括：

- **自动化部�?*：批量创建工作空间、数据存储、发布图�?
- **CI/CD 集成**：在持续集成流水线中自动配置 GeoServer
- **AI 智能�?*：通过 HTTP 接口�?AI 自主管理地图服务
- **运维脚本**：批量修改样式、更新数据源连接参数

**基础 URL 格式�?* `http://{host}:{port}/geoserver/rest`

**默认认证�?* 用户�?`admin`，密�?`geoserver`（HTTP Basic Auth�?

**数据格式�?* 支持 JSON �?XML，通过 `Accept` / `Content-Type` 请求头或 URL 后缀（`.json` / `.xml`）指定�?
...
| 操作 | 方法 | 端点 | 说明 |
|------|------|------|------|
| 获取版本信息 | GET | `/rest/about/version.json` | 返回 GeoServer 版本与组件信�?|
| 获取系统状�?| GET | `/rest/about/system-status.json` | 返回系统运行状�?|
| 获取清单 | GET | `/rest/about/manifest.json` | 返回已加载模块清�?|
| 重新加载配置 | POST | `/rest/reload` | 从磁盘重新加载配�?|
...
| 操作 | 方法 | 端点 | 说明 |
|------|------|------|------|
| 列出所有工作空�?| GET | `/rest/workspaces.json` | 返回工作空间列表 |
| 创建工作空间 | POST | `/rest/workspaces` | 创建新工作空�?|
...
| 操作 | 方法 | 端点 | 说明 |
|------|------|------|------|
| 列出所有命名空�?| GET | `/rest/namespaces.json` | 返回命名空间列表 |
| 创建命名空间 | POST | `/rest/namespaces` | 创建新命名空�?|
...
| 创建数据存储 | POST | `/rest/workspaces/{ws}/datastores` | 创建新数据存�?|
...
| 上传文件到数据存�?| PUT | `/rest/workspaces/{ws}/datastores/{ds}/file.{ext}` | 上传 Shapefile 等文件（ext: shp, gpkg 等） |
| 通过 URL 加载数据 | PUT | `/rest/workspaces/{ws}/datastores/{ds}/url.{ext}` | 从远�?URL 加载数据 |
| 外部引用数据 | PUT | `/rest/workspaces/{ws}/datastores/{ds}/external.{ext}` | 引用服务器本地文件路�?|
...
| 创建覆盖存储 | POST | `/rest/workspaces/{ws}/coveragestores` | 创建新覆盖存�?|
...
| 发布覆盖 | POST | `/rest/workspaces/{ws}/coveragestores/{cs}/coverages` | 发布新覆�?|
...
| 操作 | 方法 | 端点 | 说明 |
|------|------|------|------|
| 列出所有图�?| GET | `/rest/layers.json` | 返回所有已发布图层 |
...
| 操作 | 方法 | 端点 | 说明 |
|------|------|------|------|
| 列出图层组（全局�?| GET | `/rest/layergroups.json` | 返回全局图层组列�?|
| 列出图层组（工作空间�?| GET | `/rest/workspaces/{ws}/layergroups.json` | 返回工作空间下的图层�?|
| 创建图层�?| POST | `/rest/layergroups` | 创建全局图层�?|
| 获取图层组详�?| GET | `/rest/layergroups/{lg}.json` | 获取指定图层�?|
| 修改图层�?| PUT | `/rest/layergroups/{lg}` | 更新图层组配�?|
| 删除图层�?| DELETE | `/rest/layergroups/{lg}` | 删除图层�?|
...
| 获取样式详情 | GET | `/rest/styles/{style}.json` | 获取样式元数�?|
...
| 操作 | 方法 | 端点 | 说明 |
|------|------|------|------|
| 获取主密码信�?| GET | `/rest/security/masterpw.json` | 获取主密码状�?|
| 修改主密�?| PUT | `/rest/security/masterpw` | 更新主密�?|
...
| 列出用户 | GET | `/rest/security/usergroup/users.json` | 列出所有用�?|
| 创建用户 | POST | `/rest/security/usergroup/users` | 创建新用�?|
| 列出角色 | GET | `/rest/security/roles.json` | 列出所有角�?|

### GeoWebCache 集成

| 操作 | 方法 | 端点 | 说明 |
|------|------|------|------|
| 获取图层缓存设置 | GET | `/rest/layers/{layer}/gwc` | 获取图层�?GWC 缓存配置 |
| 修改图层缓存设置 | PUT | `/rest/layers/{layer}/gwc` | 更新图层缓存配置 |
| 清空图层缓存 (Seed) | POST | `/gwc/rest/seed/{layer}.json` | 发起缓存 seed/reseed/truncate 任务 |
| 查看缓存任务状�?| GET | `/gwc/rest/seed/{layer}.json` | 获取缓存任务进度 |
| 终止缓存任务 | POST | `/gwc/rest/seed/{layer}` | 终止正在运行的缓存任�?|
...
> 上传后自动创建同名数据存储和图层。Shapefile 需打包�?ZIP（包�?.shp�?shx�?dbf�?prj 文件）�?
...
5. PUT  /rest/layers/{layer}           �?绑定样式到图�?
```
...
3. POST .../datastores/{ds}/featuretypes        �?发布数据库表为图�?
...
请求和响应使�?JSON 时需注意 GeoServer �?JSON 包装结构�?
...
2. **recurse 参数**：删除工作空间、数据存储时，添�?`?recurse=true` 可级联删除所有子资源，否则存在子资源时删除会失败�?
3. **JSON 包装**：GeoServer REST API �?JSON 请求 / 响应体均使用单层包装对象（如 `{"workspace":{...}}`），这与标准 REST 实践不同，使用时需注意�?
4. **URL 后缀**：可使用 `.json` �?`.xml` 后缀替代 `Accept` 请求头来指定响应格式�?
5. **文件上传**：上�?Shapefile 时需打包�?ZIP 格式，且 ZIP 内必须包�?`.shp`、`.shx`、`.dbf`、`.prj` 文件�?
6. **编码**：图层名、工作空间名建议使用英文字母、数字和下划线，避免中文和特殊字符�?
7. **GeoWebCache**：通过 REST API 修改图层后，可能需要手动清空该图层�?GWC 缓存才能看到更新效果�?
...
1. **确认 GeoServer 运行**：先验证 `http://localhost:8080/geoserver/rest/about/version.json` 可访�?
2. **创建工作空间**：POST `/rest/workspaces` �?创建命名空间
3. **添加数据存储**：根据数据类�?POST datastores（矢量）�?coveragestores（栅格）
4. **发布图层**：POST featuretypes（矢量）�?coverages（栅格）
5. **配置样式**：POST styles + PUT SLD 文件 �?PUT layers 绑定样式
6. **验证发布**：GET `/rest/layers.json` 确认图层列表，访�?WMS GetMap 测试

### 关键注意事项

- **JSON 包装结构**：GeoServer REST API �?JSON 请求/响应使用单层包装（如 `{"workspace":{...}}`�?
- **recurse 参数**：删�?Workspace/Store 时务必添�?`?recurse=true` 级联删除
- **Shapefile 上传**：必须打包为 ZIP（含 .shp/.shx/.dbf/.prj），�?PUT 方式上传
- **URL 后缀**：可�?`.json` / `.xml` 后缀替代 `Accept` 头指定响应格�?
- **Content-Type**：样式上传使�?`application/vnd.ogc.sld+xml`，栅格上传使�?`image/tiff`
- **认证**：所�?POST/PUT/DELETE 操作需�?HTTP Basic Auth（默�?admin:geoserver�?
