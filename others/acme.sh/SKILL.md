...
> **官网 -- ?* <https://acme.sh>
>
> **许可证：** GPL-3.0  -- ?**最新版本：** 3.1.3 -- ?026-04）｜ **默认分支 -- ?* `master`

## 概述

acme.sh 是全球使用最广泛 -- ?ACME 客户端之一（GitHub 40k+ Star）， -- ?neilpang 发起维护，核心特点：

- **零依 -- ?*：纯 POSIX Shell 实现，无需 Python/Node/Java，只要有 `curl`/`wget`  -- ?`cron`/`crontab` 即可 -- ?
- **跨平 -- ?*：Linux 全发行版、macOS -- ?BSD、Solaris、Windows（WSL/Cygwin/Git Bash） -- ?
- ** -- ?CA**：默 -- ?ZeroSSL，可切换 Let's Encrypt、BuyPass、Google Trust Services（GTS）、SSL.com 等 -- ?
- **多验证方 -- ?*：HTTP-01（webroot / standalone / nginx / apache）、DNS-01 -- ?50+ 服务 -- ?API，支持通配符）、TLS-ALPN-01 -- ?
- **自动续期**：安装时自动写入 cron，默 -- ?60 天触发续期，支持 ARI（RFC 9773）提前续期 -- ?
- **证书部署**：内 -- ?100+ deploy hook（nginx、ssh、docker、panel、CDN、路由器等） -- ?
- **证书类型**：单域名、多域名 SAN、通配 -- ?`*.example.com`、ECC（ec-256/384）与 RSA 双证书 -- ?

> ⚠️ **不要 -- ?root 直接运行 `git` 仓库里的 `./acme.sh`**：请 -- ?`--install`  -- ?`~/.acme.sh`，由安装后的副本管理证书 -- ?cron -- ?
...
安装会做三件事：复制脚本 -- ?`~/.acme.sh/`、创 -- ?`acme.sh` 别名、写入每 -- ?cron。安装后重载 shell 或：
...
acme.sh --upgrade --auto-upgrade   # 开启自动升 -- ?
...
### 选择 CA（默 -- ?ZeroSSL，常切到 Let's Encrypt -- ?
...
```bash
# webroot 模式（已 -- ?Web 服务，最常用 -- ?
acme.sh --issue -d example.com -d www.example.com -w /var/www/html

# standalone 模式（acme.sh 临时监听 80 端口，需先停掉占 -- ?80 的服务）
...
# DNS 手动模式（无 API 时，按提示添 -- ?TXT 记录；不支持自动续期 -- ?
...
acme.sh --issue -d example.com --keylength 2048     # RSA（默 -- ?2048，可 3072/4096 -- ?
```
...
# 内置 hook 列表 -- ?deploy/ 目录：nginx、haproxy、cpanel、synology_dsm、qiniu  -- ?
```
...
- **始终 -- ?`--install-cert`/deploy hook 引用证书**，不要直接软链或引用 `~/.acme.sh/<domain>/` 下文件——这些是内部文件，路径与格式可能变化 -- ?
- **优先 DNS-01 + API**：可签发通配符且不暴 -- ?80 端口，支持全自动续期。手 -- ?DNS 模式无法自动续期 -- ?
- **生产建议指定 Let's Encrypt  -- ?GTS**，并预先 `--register-account`，避 -- ?ZeroSSL/Google  -- ?EAB 注册在续期时卡住 -- ?
- **重载命令做成幂等**，并 -- ?`--install-cert` 时配置；续期 -- ?cron `--cron` 自动完成，无需手工 -- ?
- **调试 -- ?`--staging`/`--test`**（Let's Encrypt 测试环境）避免触发签发频率限制（rate limit），再用 `--debug 2` 看详细日志 -- ?
- **多服务器场景** -- ?deploy hook（ssh/docker）集中签发、分发，避免每台机器各自验证 -- ?
...
| 问题 | 原因 / 解决 |
|------|------------|
| `Verify error: Invalid response` | HTTP-01 验证失败：检 -- ?`-w` webroot 是否正确、`/.well-known/acme-challenge/` 可被外网访问、防火墙是否放行 80 端口 |
| 通配符证书签发失 -- ?| 通配 -- ?*只能 -- ?DNS-01**，确认使 -- ?`--dns dns_xxx`  -- ?API 凭证正确 |
| `Create new order error` / rate limit | 触发 CA 频率限制；先 -- ?`--staging` 调试，或切换其它 CA |
| ZeroSSL  -- ?EAB 错误 | 先执 -- ?`acme.sh --register-account -m your@email.com` 注册 ZeroSSL 账户 |
| 续期后服务未生效 | `--install-cert` 未配 -- ?`--reloadcmd`，或重载命令报错；用 `acme.sh --renew -d xxx --force` 复现排查 |
| cron 不生 -- ?| 确认 `crontab -l`  -- ?acme.sh 条目；容 -- ? -- ?cron 环境需自行调度 `acme.sh --cron --home ~/.acme.sh` |
| `command not found: acme.sh` |  -- ?`source ~/.bashrc` 或未安装；用 `~/.acme.sh/acme.sh` 全路径调 -- ?|
...
- 用户提到「免 -- ?SSL 证书」「Let's Encrypt 自动续期」「通配符证书」「Nginx/宝塔自动 HTTPS」时，加载本技能 -- ?
- 生成命令前先确认三要素：**CA**（默 -- ?ZeroSSL，多数人想要 Let's Encrypt） -- ?*验证方式**（HTTP-01 还是 DNS-01） -- ?*部署目标**（nginx/apache/容器/远程） -- ?
- 通配符需求一律走 DNS-01 + 对应 `dns_xxx` 服务 -- ?hook，并提示用户设置对应环境变量 -- ?
- 强调“用 `--install-cert` + `--reloadcmd`”而非手动复制文件，确保续期后自动生效 -- ?
- 调试场景优先建议 `--staging`  -- ?`--debug 2`，避免命 -- ?CA 频率限制 -- ?
