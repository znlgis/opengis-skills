...
> | bitmap | [vcaesar/bitmap](https://github.com/vcaesar/bitmap) | 位图操作与图像查�?|
> | gcv | [vcaesar/gcv](https://github.com/vcaesar/gcv) | OpenCV 图像识别 |
> | imgo | [vcaesar/imgo](https://github.com/vcaesar/imgo) | 图像读写与格式转�?|
> | adb | [vcaesar/adb](https://github.com/vcaesar/adb) | Android adb 封装（移动端自动化） |

## 概述

RobotGo �?Go 生态中最成熟的桌面自动化库（GitHub 10.7k+ Star），核心能力覆盖 RPA 全链路：
...
> **v1.0.0+ 重大变更�?* v1.0.0�?025-12）起�?API 进行了重构。v1.0.2 移除了旧�?FindBitmap / OpenBitmap / SaveBitmap 等位图函数（迁移�?`bitmap` 子库）。如需�?API，使�?`v0.100.10`（最后一个保留旧位图 API 的版本）�?
...
# 需在「系统设�?�?隐私与安全性」中授予「辅助功能」和「屏幕录制」权�?

# ── Windows ──
winget install Golang.go
# GCC 工具链（三选一）：
winget install MartinStorsjo.LLVM-MinGW.UCRT   # 推荐：LLVM-MinGW UCRT
# �?winget install WinLibs.WinLibs-MINGW        # WinLibs GCC
# �?MSYS2: pacman -S mingw-w64-x86_64-gcc mingw-w64-x86_64-zlib

# ── Linux (Ubuntu/Debian) ──
sudo apt install gcc libc6-dev
sudo apt install libx11-dev xorg-dev libxtst-dev       # X11
sudo apt install xsel xclip                              # 剪贴�?
...
color := robotgo.GetPixelColor(10, 10)      // 返回 hex（不�?#�?

// 多显示器
num := robotgo.DisplaysNum()
x, y, dw, dh := robotgo.GetDisplayBounds(1)  // 第二台显示器
robotgo.DisplayID = 1                        // 设置目标显示�?
...
func main() {
    ...
    // 启动监听（阻塞）
    s := hook.Start()
    hook.Process(s)    // 处理回调，直�?hook.End()
}
...
> **gcv 依赖�?* 需要安�?OpenCV + [GoCV](https://github.com/hybridgroup/gocv)。安装方法参�?GoCV 官方文档�?
...
// 全局操作间隔（影响所有鼠�?键盘操作�?
robotgo.MouseSleep = 100                    // 鼠标操作后等�?100ms
robotgo.KeySleep = 50                      // 键盘操作后等�?50ms
```
...
1. **优先�?CaptureImg 而非 CaptureScreen**：CaptureImg 返回 `image.Image` 无需手动释放，避免内存泄漏�?
2. **中文输入用剪贴板**：`Type()` 可能被活动输入法拦截/转换。中文文本建�?`WriteAll()` + `KeyTap("ctrl", "v")`�?
3. **macOS 权限**：程序运行前必须授予终端/IDE 的「辅助功能」和「屏幕录制」权限，否则鼠标键盘静默失效、截图全黑�?
4. **位图模板需同环境截�?*：`bitmap.Find` 要求精确像素匹配，不同分辨率/DPI/主题/抗锯齿下会失败�?
5. **防无限循�?*：robotgo 模拟的键鼠事件会�?gohook 捕获。用状态标志区分「程序触发」和「用户操作」�?
6. **v1.0+ API 变更**：若�?v0.100.x 迁移，注�?`FindBitmap`/`OpenBitmap`/`SaveBitmap` 已移�?`bitmap` 子库，`GetBitmapSize` 改为 `GetImgSize(path)`�?
7. **CGO 交叉编译极困�?*：Cgo 模式需要目标平台的 C 交叉工具链和系统头文件。推荐在各平台原生构建，或使�?Cgo-free 后端�?
...
| 问题 | 原因 | 解决方案 |
|------|------|---------|
| macOS 鼠标/键盘无反�?| 未授予权�?| 系统设置 �?隐私与安全�?�?辅助功能 + 屏幕录制 |
| 截图全黑 | 未授予屏幕录制权�?| 同上 |
| `png.h not found` | 缺少 libpng 开发头文件 | Linux: `apt install libpng++-dev`；Windows: 换用 Cgo-free `-tags win` |
| Linux 剪贴板报�?| 未安�?xsel/xclip | `apt install xsel xclip` |
| Wayland 下无法工�?| Wayland 不支�?X11 协议 | 使用 `-tags wayland`（wlroots）或 `-tags libei`（GNOME/KDE�?|
| `bitmap.Find` 总返�?-1,-1 | 模板与截图环境不一�?| 确保相同分辨�?DPI/缩放/主题下截取模�?|
| Go 1.10.x 编译报错 | Go #24355 缓存 Bug | 升级 Go �?1.21+ |
| libei 后端截图失败 | libei 仅处理输�?| libei 后端不支持截�?窗口管理，使�?Cgo 模式 |

---

## RobotGo-Pro（商业版�?

RobotGo-Pro 是非开源商业版本，提供 JavaScript、Python、Lua 等多语言绑定，包含技术支持和新特性。本技能仅覆盖开�?Go 版本�?

---

## 参考资�?

- [RobotGo GitHub](https://github.com/go-vgo/robotgo) �?源码与文�?
...
- [上游中文教程](https://znlgis.github.io/others/robotgo/) �?完整 15 �?RobotGo 教程
