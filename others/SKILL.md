---
name: others-skills
description: Others（其它）技能分类索引，覆盖自托管邮件营销平台 BillionMail 与 Java 微服务后台脚手架 RuoYi-Cloud，按需加载即可获得邮件系统搭建、Spring Cloud 微服务开发等通用工具的精准 AI 辅助。
tags:
  - email
  - self-hosted
  - newsletter
  - java
  - spring-cloud
  - microservices
  - scaffold
---

> **父级入口：** [../SKILL.md](../SKILL.md) — 全仓 56 技能总索引

## 概述

本分类当前涵盖 **2 个其它技能文件**，收录不归属于 GIS、CAD、C#、AI、IoT 的通用开源工具。

### 何时加载此索引？

- 需要自建邮件营销 / 事务邮件 / Newsletter 系统
- 需要 Java 微服务后台管理脚手架（Spring Boot 3 + Spring Cloud）
- 寻找可替代商业方案的开源自托管工具

---

## 技能列表

| 技能 | 简介 | 关键标签 |
|------|------|---------|
| [billionmail](./billionmail/SKILL.md) | 自托管邮件营销与事务邮件平台 | `email` `selfhosted` `marketing` |
| [ruoyi-cloud](./ruoyi-cloud/SKILL.md) | 若依微服务版 Java 后台脚手架 | `java` `springcloud` `scaffold` |

---

## 快速导航

| 用户需求 | 推荐加载 |
|---------|---------|
| "自建邮件群发 / Newsletter 平台" | `billionmail/SKILL.md` |
| "配置 SMTP 中转与送达率分析" | `billionmail/SKILL.md` |
| "搭建 Spring Cloud 微服务后台" | `ruoyi-cloud/SKILL.md` |
| "Nacos/Sentinel/Seata 微服务脚手架" | `ruoyi-cloud/SKILL.md` |

---

## 相关分类

- **[csharp/](../csharp/SKILL.md)** — .NET 后台框架与脚手架（Admin.NET、Furion）
- **[ai/](../ai/SKILL.md)** — LLM 应用与 Agent 框架
- **[gis/](../gis/SKILL.md)** — GIS 数据处理与地图服务
