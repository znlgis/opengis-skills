...
robotgo-flow 是一款基�?[robotgo](https://github.com/go-vgo/robotgo) 实现�?**Windows 桌面 RPA 自动化框�?*。通过 YAML 声明式描述自动化流程，用图像模板匹配定位屏幕 UI 元素，自动执行鼠标、键盘、浏览器等操作，**无需编写 Go 代码**�?

- **Go CLI**：纯命令行工具（`run` / `record` / `capture` / `serve`），专注自动化逻辑�?
- **WPF 托盘应用**：基�?.NET 10 WPF �?Windows 托盘应用，实时进度监控与任务栏通知（可选组件）�?
- **图像模板匹配**：预截取 UI 元素截图定位目标，适应窗口位置变化；窗口内优先搜索、全屏回退�?
- **交互式动�?*：运行时输入框（支持密码隐藏）、确认对话框、系统通知，可实现人工决策节点�?
- **运行时变量注�?*：`$input.<name>` 占位符，运行时动态替换文本�?
- **人类行为模拟**：贝塞尔曲线鼠标轨迹、打字错误与修正、可变延迟、空闲抖动，降低被检测风险�?
- **交互式录制器与截图工�?*：CLI 逐步引导录制工作流、框选截图�?
- **容错与调�?*：每步自动截图、从指定步骤恢复、abort/skip/retry 三种错误策略、context 安全取消�?
- **GBK/UTF-8 自动编码**：Windows 中文环境下自动处理路径与 YAML 编码�?

> 当前仅支�?Windows 平台。命令与字段以仓库最新代码为准�?
...
> 首次编译�?4 分钟（需编译 GLFW C 源码），后续增量编译�?1 秒�?
...
```powershell
robotgo-flow                       # 无参�?�?显示帮助
robotgo-flow run <工作流文�?      # 执行工作�?
robotgo-flow record                # 交互式录制工作流
robotgo-flow capture [元素名]      # 交互式截取模�?
robotgo-flow serve <工作流文�?    # JSON-Line 协议服务（供 WPF GUI 调用�?
```
...
| `--debug` | false | 每步自动保存截图到输出目�?|
| `--out dir` | 工作流所在目�?| 截图输出目录（默�?`screenshots/`�?|

```powershell
./robotgo-flow.exe run workflow.yaml
./robotgo-flow.exe run workflow.yaml --from 3     # 从第 3 步开�?
./robotgo-flow.exe run workflow.yaml --debug      # 调试模式
```

**`record` 参数�?* `--out`（默�?`workflow.yml`）、`--tpl-dir`（默�?`./templates`）�?
**`capture` 参数�?* `--out-dir`（默�?`templates`）�?
...
settings:                      # 可选：全局设置
  element_timeout: 10          #   等待元素超时秒数（默�?10�?
  on_error: abort              #   错误策略：abort / skip / retry（默�?abort�?
  max_retries: 3               #   retry 模式最大重试次数（默认 3�?
  browser_refresh_delay: 3     #   刷新页面等待秒数（默�?3�?
  browser_navigation_delay: 2  #   前进/后退等待秒数（默�?2�?
  browser_page_load_delay: 3   #   打开 URL 等待秒数（默�?3�?
  human:                       #   人类行为模拟
    enabled: false             #     是否启用
    speed: 1.0                 #     速度系数�?.1~5.0，默�?1.0�?
    mistake_rate: 0.03         #     打字错误率（0.0~1.0，默�?0.0�?
steps:                         # 必填：步骤列�?
  - name: "步骤�?
    actions: [...]             #   动作列表
```
...
|----|------|
| `abort` | 立即终止执行（默认） |
| `skip` | 记录错误日志，跳过当前动作，继续下一�?|
| `retry` | 重试当前动作，最�?`max_retries` 次；耗尽�?abort |
...
1. 预先截取目标 UI 元素截图（PNG），保存�?`templates/`�?
2. 运行时通过 `robotgo.FindBitmap()` 在屏幕搜索模板�?
3. 定位成功后，在匹配位置中心执行鼠�?键盘操作�?
...
| 行为 | 说明 |
|------|------|
| 贝塞尔曲线鼠标轨�?| 自然曲线路径，靠近目标减�?|
| 打字错误模拟 | �?`mistake_rate` 引入相邻键错�?漏字/顺序颠倒并自动修正 |
| 可变延迟 | 操作间隔在随机范围内变化 |
| 滚动抖动 | 分块滚动，概率轻微反向回�?|
| 空闲行为 | 长时等待时鼠标微抖动、步骤间概率移动到随机位�?|
...
- `internal/action` 定义 `Runner` �?`Engine` 接口，作为引擎与执行器契约；`factory.go` �?`FromConfig` �?`config.Action` 映射�?`Runner` 实现—�?*扩展新动作类�?*即在此新增�?
- WPF 托盘应用 (`src/csharp`) 通过 `robotgo-flow serve` 子进�?+ JSON-Line stdin/stdout 协议通信�?
...
- 生成工作流时优先�?*模板匹配**（`templates/*.png`）而非硬编码坐标，以适应窗口移动�?
- 涉及登录/密码等敏感输入，�?`inputs` + `$input.*` + `mask: true`，不要把凭证写进 YAML�?
- 不稳定步骤设�?`on_error: retry` + `max_retries`；调试用 `run --debug` 逐步截图�?
- 需要拟人化/反检测时启用 `settings.human`�?
- �?Windows 平台；提醒用�?Go `1.26+` �?MinGW-w64 工具链是编译前提�?
...
| 问题 | 说明 |
|------|------|
| 模板匹配不到元素�?| 确认截图清晰、缩放一致；浏览器场景确保目标在窗口内；必要时重�?`capture`�?|
| 首次编译很慢�?| 需编译 GLFW C 源码，约 4 分钟；后续增量编译约 1 秒�?|
| 中文路径乱码�?| `internal/encoding` 自动处理 GBK↔UTF-8，无需手动干预�?|
| 一定要 WPF 应用吗？ | 不需要；Go CLI 可独立使用，不依�?.NET/WPF�?|
| 如何从中途恢复执行？ | `run --from N`�?-indexed）�?|
...
- 上游中文教程�?9 章）�?https://znlgis.github.io/others/robotgo-flow/>
